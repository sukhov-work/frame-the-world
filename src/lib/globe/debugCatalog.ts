...
export const DEBUG_METRICS: ReadonlyArray<DebugMetricDef> = [
  ...
  },
  {
    ...
    group: "renderer",
    fmt: "bool",
    note: "UnrealBloomPass.enabled — tier-gated AND flat-chart-gated per frame (13 draws + 12 clears appear/vanish on the flip; only the additive blend is full-res).",
  },
  {
    id: "canvas.bloomScale",
    label: "bloom scale",
    group: "renderer",
    fmt: "float2",
    note: "T80 mip-chain resolution scale (ScaledBloomPass). 1 = the stock pass, forwarded untouched; 0.5 = a quarter of the bloom fill. Follows the tier unless __quality.bloomScale() pins it.",
  },
  {
    id: "canvas.gtao",
    label: "GTAO pass",
  ...
  {
    id: "ultra.shadow.intensity",
    label: "shadow field",
    group: "shadow",
    fmt: "float2",
    spark: true,
    note: "sunLight.shadow.intensity — three's mix(1.0, shadow, k), the ONE write that fades building-received shadows and the ground twins together. The sunset release cliff was invisible because nothing published it.",
  },
  {
    id: "ultra.shadow.groundOpacity",
    label: "ground overlay",
    group: "shadow",
    fmt: "float2",
    note: "The shared ShadowMaterial's LIVE opacity (read off the material). Deliberately stale below the elevation gate — setShadowStrength only runs while the sun casts, and castShadow false makes the mask 1 anyway.",
  },
  {
    id: "ultra.shadow.directShareK",
    label: "overlay direct share",
    group: "shadow",
    fmt: "float2",
  ...
  {
    id: "ultra.shadow.rig.refreshes",
    label: "rig refreshes",
    group: "shadow",
    fmt: "int",
    note: "CUMULATIVE depth-map re-renders of cascade 0 (T77 A2). Difference two reads. Equal to the frame count = the rig is not demand-driven; climbing far slower = the texel quanta are holding.",
  },
  {
    id: "ultra.shadow.rig.ageMs",
    label: "rig age",
    group: "shadow",
    fmt: "ms1",
    budget: 1000,
    warnAbove: 3000,
    note: "ms since cascade 0's depth map was re-rendered (extent / terrain epoch / eye drift / key swing, else the SHADOWS.rigMaxStaleMs net). An age that only climbs = the refresh policy is dead.",
  },
  {
    id: "ultra.shadow.rig.snapTexels",
    label: "rig snap",
    group: "shadow",
    fmt: "float2",
    note: "How far the last refresh slid the box to land on the light-plane texel lattice, in texels. Each in-plane component rounds by at most half a texel, so this is bounded by √2/2 ≈ 0.71 by construction.",
  },
  {
  ...
  {
    id: "tiles.gnd.canvasReleased",
    label: "composite canvases released",
    group: "tiles",
    fmt: "int",
    note: "T123 lever (d) (2026-09-10): composite imagery canvases whose pixel backing store was dropped (width = height = 0) at the overlay library's own dispose instead of at the element's GC. ~450 are born per /m stress cycle (256² lean / 512² high). Climbs with imagery turnover on both shells; the iPhone is the tier that pays (an accelerated canvas is an IOSurface in the footprint jetsam reads).",
  },
  {
    id: "tiles.gnd.canvasReleasedMB",
    label: "composite canvas MB released",
    group: "tiles",
    fmt: "mb1",
    note: "T123 lever (d): the summed RGBA backing-store bytes of the composites released since boot (width × height × 4 each). On the twin: ~450 MB per stress cycle at 512², the store the library had left to the collector.",
  },
  {
    id: "tiles.gnd.canvasSkipped",
    label: "composite disposals skipped",
    group: "tiles",
    fmt: "int",
    note: "T123 lever (d): overlay disposals that carried no canvas — the single-tile FAST PATH hands out an ImageBitmap clone (closed by the tiled source itself), or a cancelled fetch's null. Informational.",
  },
  {
    id: "tiles.gnd.dlLen",
    label: "gnd ⬇ queue",
  ...
  {
    id: "terrain.stream.priorityOn",
    label: "img priority on",
    group: "imagery",
    fmt: "int",
    note: "1 = the imagery-fetch ordering is installed (lib/globe/overlayFetchPriority): parse-slot tiles first, downloaded tiles by error, preloads last. 0 = the library drifted or GROUND.overlayFetchPriority is off — the plugin's own FIFO below every terrain download.",
  },
  {
    id: "terrain.stream.imgTagged",
    label: "img fetches tagged",
    group: "imagery",
    fmt: "int",
    rate: true,
    note: "Imagery GETs that carry a live tile-following priority through the ground download queue.",
  },
  {
    id: "terrain.stream.imgUntagged",
    label: "img fetches untagged",
    group: "imagery",
    fmt: "int",
    note: "Imagery GETs queued with no tile context (the failed-overlay re-lock) — kept at the library's FIFO value. Should stay near 0.",
  },
  {
    id: "terrain.stream.slotBoosts",
    label: "slot boosts",
    group: "imagery",
    fmt: "int",
    rate: true,
    note: "Imagery GETs re-ranked into the parse-slot band because a parse slot was waiting on them — each one is a fetch that under the old FIFO would have held a slot behind the whole image queue (the 38 s stall, counted).",
  },
  {
    id: "terrain.stream.fullCacheKicks",
    label: "full-cache kicks",
    group: "imagery",
    fmt: "int",
    rate: true,
    note: "Re-traversals forced while the ground cache was full and the queues idle (GROUND.fullCacheKickMs): the library drops a tile parsed onto a full cache without a needs-update, and nothing re-traverses while the camera is still. A steady rate at rest = the working set exceeds the cap (the tier's groundLruBytesMB).",
  },
  {
    id: "terrain.split.calls",
    label: "virtual splits",
    group: "imagery",
    fmt: "int",
    rate: true,
    note: "expandVirtualChildren calls on the ground overlay plugin (a leaf terrain tile clipped into virtual children so its composite can follow the lens). Main-thread, synchronous.",
  },
  {
    id: "terrain.split.growthCapped",
    label: "splits refused (growth)",
    group: "imagery",
    fmt: "int",
    warnAbove: 0,
    note: "Virtual splits refused because the child carried MORE triangles than the parent it was cut from — the runaway signature (cost doubling per level, 0.7 → 95 s). Any count is the guard earning its keep (lib/globe/virtualSplitGuard).",
  },
  {
    id: "terrain.split.timeCapped",
    label: "splits refused (slow)",
    group: "imagery",
    fmt: "int",
    warnAbove: 0,
    note: "Virtual splits refused under an ancestor whose own split exceeded GROUND.virtualSplitMaxMs.",
  },
  {
    id: "terrain.split.depthCapped",
    label: "splits refused (depth)",
    group: "imagery",
    fmt: "int",
    note: "Virtual splits refused past GROUND.virtualSplitMaxDepth levels below a real tile.",
  },
  {
    id: "terrain.split.worstMs",
    label: "worst split ms",
    group: "imagery",
    fmt: "ms1",
    warnAbove: 80,
    note: "The slowest single virtual split this session. Healthy: ≤ ~17 ms on a 2,500-triangle leaf.",
  },
  {
    id: "terrain.split.totalMs",
    label: "split ms total",
    group: "imagery",
    fmt: "ms1",
    note: "Main-thread time spent in virtual splits this session.",
  },
  {
  ...
  },
  {
    ...
    fmt: "int",
    rate: true,
    note: "Monotone load-model counter shown as Δ/s — the streaming pulse (absolute value is meaningless). Still the ULTRA cascade + BEST SPOT refresh trigger; since T77 lever 6 it no longer drops the height memo.",
  },
  {
    ...
    fmt: "int",
    rate: true,
    note: "Terrain-height memo hits Δ/s (RC11). Freshness is per-TILE since T77 lever 6 — arrival/disposal drops only the buckets that tile covers — so a hit is still exactly as fresh as a raycast. Capacity 250k entries.",
  },
  {
  ...
  },
  {
    ...
    fmt: "int",
    budget: 250000,
    note: "Live memo size vs the 250k capacity. Since T77 lever 6 the working set ACCUMULATES (it is no longer wiped several times a second), so this settles near the city's real footprint count; overflows climbing = the capacity is too small for it.",
  },
  {
    id: "terrain.memo.regionInvalidations",
    label: "memo tile drops",
    group: "terrain",
    fmt: "int",
    rate: true,
    note: "T77 lever 6: per-TILE memo invalidations Δ/s — one per arriving/disposed terrain tile whose region held live entries. It should track the tile pulse; if it stays at 0 while terrain streams, the spatial path is not engaging and the memo is running on the maxAge backstop alone.",
  },
  {
    id: "terrain.memo.fullDrops",
    label: "memo full drops",
    group: "terrain",
    fmt: "int",
    warnAbove: 4,
    note: "T77 lever 6: whole-map memo drops that were NOT capacity — a tile region too wide for the bucket walk, or the 60 s staleness backstop. A handful per session is the backstop doing its job; a climb means coarse ancestors are being invalidated wholesale and lever 6 is buying nothing.",
  },
  {
    id: "terrain.memo.regionless",
    label: "memo region-less tiles",
    group: "terrain",
    fmt: "int",
    warnAbove: 0,
    note: "T114 (2026-09-07f): terrain-tile events (arrival or disposal) that carried no usable `boundingVolume.region`, so the height memo dropped nothing and only the 60 s staleness backstop refreshes it. The CWT quantized-mesh path always carries a region; any count here names a tile source that does not (the ESRI placeholder path was the unverified one).",
  },
  {
    id: "terrain.memo.staleMismatches",
    label: "memo stale hits",
    group: "terrain",
    fmt: "int",
    warnAbove: 0,
    note: "T77 lever 6 falsification counter (DEV only): memo hits that DISAGREED with a fresh raycast by >1 cm. Must be 0 — anything else means per-tile invalidation under-drops and something is seated on terrain that no longer exists.",
  },
  {
  ...
  {
    id: "buildings.collapsed",
    label: "seat collapses",
    group: "buildings",
    fmt: "int",
    rate: true,
    note: "T77 step 4a: APPLY-time poisoned-pair collapses Δ/s — a building or tree that WAS seated being dropped back onto the cell plane because its seat became implausible against a corrected cell seat. Unlike 'seat rejections' (a discarded sample, nothing moves) each of these is a visible vertical jump.",
  },
  {
  ...
  },
  {
    ...
    label: "enriched load units pending",
    group: "buildings",
    fmt: "int",
    note: "T106 (b): ENRICHED meshes whose `load-model` PHASE 2 (crease edges → per-building attribution → feature registry / banked seats / overrides) is still on the deferred queue. Climbs on a landing burst, drains under ENRICHED.loadBudgetMs per frame (6 desktop / 3 lean), nearest cell first; a value that never reaches 0 means the budget is starved or a unit is stuck.",
  },
  {
    id: "buildings.loadMaxMs",
    label: "enriched load drain max ms",
    group: "buildings",
    fmt: "ms1",
    budget: 6,
    warnAbove: 16,
    note: "T106 (b): the WORST single per-frame drain of the ENRICHED deferred `load-model` queue (ms) since attach. Should sit at the budget plus one atomic unit (an attribution or a registry pass); the pre-slice handler did the whole cell in one frame — 65 ms for the biggest Dnipro cell on the phone twin.",
  },
  {
    id: "buildings.osmLoadPending",
    label: "OSM edge units pending",
    group: "buildings",
    fmt: "int",
    note: "T106 OSM (2026-09-07f): OSM b3dm meshes whose crease edges are still on the OSM handler's deferred queue (PHASE 2 of its `load-model`). Drains in `buildings.update()` under BUILDINGS.loadBudgetMs per frame (3 desktop / 1.5 lean), nearest tile first — a separate budget from the enriched queue's, one frame apart.",
  },
  {
    id: "buildings.osmLoadMaxMs",
    label: "OSM edge drain max ms",
    group: "buildings",
    fmt: "ms1",
    budget: 3,
    warnAbove: 16,
  ...
  {
    id: "buildings.detachedReleases",
    label: "detached-cache drains",
    group: "buildings",
    fmt: "int",
    note: "T123 lever (a) (2026-09-10): how many times the /m 2D map DRAINED the detached enriched + OSM tile caches since boot (once per detach period, MOBILE2D.releaseDetachedGraceMs after the detach). A detached TilesRenderer never runs its LRU, so before the lever every FPV spot's cells stayed resident until the iPhone's 2 GB ceiling. Always 0 on the desktop (it never detaches through this path).",
  },
  {
    id: "buildings.detachedReleasedMB",
    label: "drained tile MB",
    group: "buildings",
    fmt: "mb1",
    note: "T123 lever (a): cumulative byte-accounted content the drains evicted (the caches' own `cachedBytes`; the renderer's real footprint per tile-MB is ~3.9× that — T83). Read with `tiles.enr.lruItems` / `tiles.bld.lruItems`, which must sit at 0 on the resting 2D map.",
  },
  {
    id: "buildings.detachedReleasedItems",
    label: "drained tiles",
    group: "buildings",
    fmt: "int",
    note: "T123 lever (a): cumulative items evicted by the drains — loaded cells, in-flight downloads (aborted through the library's own controller) and queued requests alike.",
  },
  {
    id: "buildings.detachedReleaseLeft",
    label: "tiles left after drain",
    group: "buildings",
    fmt: "int",
    warnAbove: 0,
    note: "T123 lever (a): items BOTH caches still held right after the last drain. Must be 0 — the drain zeroes all four caps for one synchronous `unloadUnusedContent()`, whose first loop evicts every unused item. Non-zero = the library's eviction loop changed shape (the unit tier pins 0.4.28).",
  },
  {
    id: "buildings.detachedReleaseMaxMs",
    label: "drain max ms",
    group: "buildings",
    fmt: "ms1",
    budget: 8,
    warnAbove: 33,
    note: "T123 lever (a): the WORST single drain (ms) since boot — the whole cache's `dispose-model` work (geometry/material/texture disposal, the enriched seat banking) lands in ONE frame, on the resting 2D map past the exit flight. Judged on the farm iPhone; a drain above a frame at rest is the case for a chunked eviction.",
  },
  {
