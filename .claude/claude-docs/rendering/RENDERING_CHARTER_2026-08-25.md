...
| RC16 | Straddler rule unified + margin/crossfade ring (audit S19) — ×3 bakes | [BAKE] | M | **SHIPPED 2026-08-26d, and the margin ring was NOT needed.** `scripts/bake/measure-straddlers.mjs` showed `droppedOutside` was two populations under one name: 96–99.9 % of it is DISJOINT geometry from OSM2World's relation-recursed extract (median 761 m / 40 km / 36 km out, max 55/653/149 km), correctly dropped; only **123 / 61 / 1** features were true straddlers, and dropping THOSE left a half-building notch where the prism had already erased their Cesium twin's inside half. One rule in `lib/geo.mjs` (`binFeature`): ownership by bbox INTERSECTION, placement by clamped centroid. Both bakers now agree; the extruder's output is byte-identical (its FAR count is 0 — `way["building"](bbox)` already scoped its harvest). No Overpass re-fetch, no re-convert, no `marginM`, no `regions.ts` change. **Residual, measured and NOT fixed:** an owned feature still pokes outside the prism where Cesium also draws it — a coincident sliver on 0.03–0.42 % of features, median 3–10 m. That is the crossfade ring's job; growing the prism instead punches a hole. |
...
| RC21 | On-demand render (RENDERING_QUALITY_PASS P11): skip `composer.render()` when nothing animates. | [ADD] | M–L | **SHIPPED OFF 2026-08-26d** (`lib/globe/frameGate.ts` + `GATE` + `window.__frameGate`). The charter's premise — a predicate covering every animating source — is refused on the recon's own evidence (40+ sources / 20 files / ~14 no-snap eases; a false negative is a FROZEN GLOBE). Built heartbeat-first on `pipCache`'s shape instead: `GATE.maxStaleMs` (200 ms) is the safety net, the epsilons are only an optimisation, and a **settle window** `restMs` = 6.2 × `ULTRA.exposureTauMs` = 6,000 ms keeps the renderer at full rate while any ease converges. `markGateDirty()` covers tier applies / bloom flip / ULTRA chip / resize / context-restore; **tile streaming and the StylizedTiles eases are covered by the heartbeat ALONE** — by design, and the reason the heartbeat may not be raised. Two rollbacks (`enabled: false`, `maxStaleMs: 0`) are unit-LOCKED. It buys **GPU and power, not CPU** — `tilesHandle.update()` still runs every frame. **The default flip is an owner call and wants an ULTRA timelapse soak — browser-UNVERIFIED.** |
