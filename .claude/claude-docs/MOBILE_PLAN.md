...
**2026-09-25 (owner order after the iPhone 17 Pro test — a UX-only pass, no behaviour change): the TAB BAR is ONE LINE per tab (glyph beside label, 49 → 27 px, fonts 0.6 → 0.56rem / glyph 0.95 → 0.85rem; the row is still the whole tap target) and every seat above the bottom column dropped by the same 1.2rem (`.m-actions` 9.6rem, the sticks 10rem, `--m-altcol-bottom-base` 15.2rem); the two 108 px sticks are SEE-THROUGH (24 % tint + a 2 px blur, was 55 % + 6 px — same size, same rim; the focal mm stands on its own 72 % pill); ⌖ ALIGN left the right-rail float for a round cell on the LEFT rail above the AIM stick (the column's TOP cell while calibrating); CONFIRM is the BOTTOM verdict cell, nearest the stick (CANCEL · RESET · CONFIRM); the 3D ↔ CAM slider reads the mini-map as rendered (folded = the card's 88 px back). DECISIONS 2026-09-25.** **2026-09-22 (owner order, the small-screen pass): the calibration strip is ONE row (title + `YAW · PITCH · mm`, no memo; the mini-map folds to its puck while calibrating), CONFIRM / RESET / CANCEL are three ROUND cells on the LEFT rail just above the AIM stick, the 3D ↔ CAM slider stands VERTICAL above the CAM chip; ROLL left the calibration (the sensed roll still levels the feed), a pinch OUT enlarges the feed; the sun / moon / target are ringed ABOVE the feed with thick day arcs (`scene/arGuides.ts`) so the sky is the calibration target; `/m` boots the whole planet even from the desktop's 1,100 km hand-over hash. THE RULE FOR EVERY /m SURFACE FROM HERE (owner): "we have too little screen space and must use it efficiently" — a control goes where the thumb is (the rails), never a row across the top; readouts share a line; anything that only reads is a pill, anything that acts is a 44 px cell.** **2026-09-19 (owner order): AR is GYRO-LED with a slow compass TRIM on both platforms (`lib/sensors/yawTrim.ts` — the swing no longer reaches the view); the altitude column gains a FOURTH cell while AR is on — `CAM`, the phone's rear camera over the 3D view at the same pixel focal (`mobile/ArCameraOverlay.tsx`, a CSS-opacity feed at z 1, the canvas stays opaque; `--m-altcol-h` 148 → 200 px via `:has(.m-cambtn)`); a LONG PRESS on AR opens VISUAL CALIBRATION (drag = yaw/pitch, twist = roll, pinch = the camera's FOV; ✓ CONFIRM → `ftw:ar-calib:v1`, ↺ RESET, ✕ CANCEL; the yaw is a compass BIAS inside the trim); `controls/useLongPress.ts` is the shared hook (T145). On /m the overlay-resolution raise is taken on frame 1 (no more white ground at the FPV exit). The 2026-08-11 "upload permanently out of /m" ruling STANDS — the ▣ ADD A BOX primitive ships in the UPLOAD dialog only (T148 asks). DECISIONS 2026-09-19; ARCHITECTURE §7f; harness `scripts/verify-ar-calibration.mjs` 52/52; the DEVICE pass is T147.** **2026-09-08b (owner order, five asks): the 🧭 AR chip is the FIRST 44 px cell of the right-rail altitude column with a TRANSIENT bubble (rung/stale transitions only); 💾 SAVE is a 44 px icon cell, `aria-disabled` signed-out with a tap hint; FIND / SPOT tabs are LIVE (accent dot) while working and a LONG PRESS toggles them (`mobile/tabLive.ts`); BEST SPOT's lift is debounced, its disarm cancels + the worker idles out (T121), the sheet altitude is the shared `controls/RateEncoder` on both shells; the two-finger TWIST rotates the map with the fingers (T120, `lib/globe/twistTracker.ts`). DECISIONS 2026-09-08b; harness `scripts/verify-mobile-batch-2026-09-08.mjs` 109/109.** **2026-09-07h (owner order 2026-09-07g): two mobile features landed — BEST SPOT as the fifth `/m` tab (`◎ SPOT`, `mobile/BestSpotSheet.tsx`, no ULTRA) and AR LOOK-AROUND in mobile FPV (`mobile/FpvControls.tsx` `ArLookToggle` → `scene/arLook.ts` → `lib/sensors/*`); DECISIONS 2026-09-07h, backlog T116–T118.** **Status (refreshed 2026-08-15): M0–M3 COMPLETE (shell + planning sheets + FPV touch + the
M3a/M3b/M3c planning twins; dated lines in the §Decision-log tail) · M4 (events twin) is next,
QUEUED BEHIND the guide track (`archive/GUIDE_PLAN.md`, G1+G2 shipped; archived 2026-08-18).** **RE-RULED 2026-08-13 (owner):** the §5 feature ladder
...
| 9 | **Lunar eclipses** — ✅ SHIPPED 2026-08-22k (both shells; solar too) | M | shadow-cone geometry from existing sun/moon; local visibility = existing horizon test. (Solar-eclipse umbra path on the globe = flagship visual, L, later; Besselian elements or astronomy-engine's eclipse search) |
