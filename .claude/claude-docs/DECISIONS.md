...
- **2026-09-25b · RELEASED — the 2026-09-25 /m UX pass is LIVE as v1.36.21** (owner: "in the end make sure to commit and release everything"). **Ship:** the hook in the foreground (`FTW_SHIP_DAEMON=1`): gates green → `claude/ship-20260925-022953` → squash-landed on origin/master as **`6b791a4`** (PR #134) after 60 s, local checkout back on master, `package.json` 1.36.20 → 1.36.21, no SHIP_ATTENTION. **`release:full`** (started in the background with its log in the scratchpad — the whole run is ~5 min, a 249-char comment counted first): vitest 221 files + astro 0/0 green again → `wix build` → `wix release` "Site published on plux.today" → canary **GET 200 · POST 200** → warm **169 assets, 0 failed, 0 cold** → `verify-prod-globe` **canvas 1728×993 gl=true** (`verify-shots/release-20260925-023350.jpeg`), the only console lines the library's tiles-1.1 notice ×2 and the anonymous 403/401 pair (as every release since 2026-09-17c). **The edge carries the pass** (fetched from `/m`): `MobileShell.C9TukMFw.css` has `.m-aralign`, `.mm--collapsed` and six `--m-armix-h` reads; `MobileShell.CRGC1T73.js` the **1.36.21** stamp, `data-act="ar-align"` and `ar-cal-cancel` first in the column. No `/api` write changed → `verify-live-deletes` not owed. House Chrome closed, no dev server. **Harness hygiene learned:** `verify-mobile-fixes-2026-09-18` takes `[port] [shotsDir]` — a `--shots` flag becomes a folder named `--shots` in the repo root (moved under `verify-shots/`); its 3j check pinned `16.4rem + 52px` and is re-pinned to 15.2rem. **The owner tests v1.36.21 on the iPhone 17 Pro next** (the ALIGN cell, CONFIRM under the thumb, the pads' see-through level, the 27 px tab row; the two open calls — hide ALIGN while calibrating? auto-fold the mini-map while CAM is on?).
- **2026-09-25 · THE /m UX PASS AFTER THE iPHONE 17 PRO TEST** (owner order with a screenshot of v1.36.19's calibration screen: "a very small session, specific mobile fixes, purely UX, be careful not to regress"; `/frame` fix-mode, no research agents; leaf `mem:project/wip-2026-09-25-mobile-ux-pass`; a scratchpad probe measured the 402 × 714 and 402 × 874 twins before / after — `verify-shots/ux-{before,after}-*.jpeg`). **(1) ⌖ ALIGN → the LEFT rail** (`FpvControls.tsx` `ArLookToggle`, fpv.css `.m-aralign`): the chip floated above the right-rail column (on top of the mini-map card at 714 px; the slider "crushed" under it) — now a 44 px `.m-act--icon` cell (⌖ over ALIGN, `data-act="ar-align"`) FIXED at the verdict column's seat (`10rem + 120px + 110px + 12px`), the column's TOP cell while calibrating (`body.m:has(.m-arcal__actions) .m-aralign` +156 px), hidden with the map open like the float it left; it stays `.m-arwrap`'s child for the "ALIGNED" flash (fixed geometry ignores the wrap). What it is: on a gyro-only rung the tap ties the CURRENT phone pose to the camera heading (`alignEpoch` → `arLook.ts` `ladder.align`). Why it was on the owner's calibration screen: aiming at the sky closes the iOS compass gate (`orientationLadder` rung 2 `compassMinTopHoriz` / `compassMinScreenUp`) → `relative-*` → the chip; there CONFIRM is already "an ALIGN by eye" (`arLook.ts` the commit branch) — hiding it while calibrating is an OWNER CALL, not done (T149 addendum). **The 3D ↔ CAM slider reads the mini-map as RENDERED**: `body.m:has(.m-arcal__mixv):has(.mm--collapsed)` reserves the 46 px puck instead of the 134 px card — on the 714 twin 55 → **128 px** calibrating (the card always folds there) and 55 → 74 in CAM view (the seat drop below). **(2) CONFIRM nearest the stick**: the column is CANCEL · RESET · CONFIRM top-down (was CONFIRM first); `verify-ar-calibration` (`ui.acts.join()`) and `mobileArCamera.test` read the order — updated. **(3) SEE-THROUGH STICKS** (fpv.css `.m-joy`): fill 55 → **24 %**, blur 6 → **2 px**, rim 35 → 40 % — same 108 px, same knob; the focal readout `.m-joy__footer` stands on its own 72 % pill (owner: "do not make the focal value transparent"). **(4) THE TAB BAR IS ONE LINE PER TAB** (chrome.css `.m-tab` row: glyph beside label, 0.56rem / glyph 0.85rem, padding 0.3rem 0 0.55rem; the row's own bottom pad is the safe-area only, so the whole row stays the tap target): **48.6 → 27.2 px (56 %)**, the bottom column 166 → 145 px, the build stamp still clears (8.8 px ≥ 8.2). **Every seat above the column dropped 1.2rem** so the saved band reaches the view and the rails: `.m-actions` 10.8 → **9.6rem**, `.m-joy` 11.2 → **10rem**, `.m-joy--aim-fpv` 10rem + 120px, `.m-arcal__actions` 10rem + …, `--m-altcol-bottom-base` 16.4 → **15.2rem** (map-window.css fallback too); the chips' gap to the column 6.7 → 8.9 px (no overlap). Docs: ARCHITECTURE §7a′, `MOBILE_PLAN.md` header, the guide step ("just above the aim stick"), `mobileFpvRail.test` / `verify-mobile-fixes` 2g (15.2rem). **Traps:** `pinchHardening.test` reads the FIRST occurrence of an audited selector in the file — a comment naming `.m-actions` above its rule fails it; `mobileArLook.test` must slice the line-start `.m-aralign {` (the `mw-open` hide list names it first). **Verification.** vitest **3,276 / 221 files** (unchanged count; the touched files re-pinned) · astro **0/0/12** · browser (dev, house Chrome): `verify-ar-calibration` **66/66** · `verify-ar-look` **39/39** · `verify-mobile-batch-2026-09-08` **127/127** · `verify-mobile-fixes-2026-09-18` **ALL PASS (45; its 3j check re-pinned to the 15.2rem base)** · `verify-fpv-carry-2026-09-22` **29/29**; no GL change → no pose sweep owed. **Open for the owner:** hide ALIGN while calibrating? auto-fold the mini-map while CAM is on (the slider takes the room in VIEW mode too)?
- **2026-09-22b · RELEASED — the 2026-09-22 batch is LIVE as v1.36.19** (owner: "deploy and finalize session, I will test on devices"). **Ship:** the hook in the foreground (`FTW_SHIP_DAEMON=1`): gates green → `claude/ship-20260922-093333` → squash-landed on origin/master as **`4ac168c`** (PR #132) after 60 s, tree-proven (`git diff afc6399 origin/master` empty), mirror synced, local checkout back on master, `package.json` 1.36.18 → 1.36.19, no SHIP_ATTENTION. **`release:full`:** run 1 FAILED at its own comment gate — **the release comment is capped at 250 characters** (mine was 294; a second attempt at 252 was stopped before the gates) → run 3 with 244: vitest 221 files + astro 0/0 green again → `wix build` → `wix release` "Site published on plux.today" → canary **GET 200 · POST 200** → warm **169 assets, 0 failed, 0 cold** → `verify-prod-globe` **canvas 1728×993 gl=true** (`verify-shots/release-20260922-093911.jpeg`), the only console lines the library's tiles-1.1 notice ×2 and the anonymous 403/401 pair (the same as every release since 2026-09-17c). **The deployed bundles carry the batch** (fetched from the edge): `MobileShell.BuvQYDjs.js` has the **1.36.19** stamp + `m-arcal__mixv` / `m-arcal__actions`, `MobileShell.CYU98wKQ.css` their rules, `MiniMap.BMfVtI6q.js` the `arCam==="calibrate"` fold, `tuning.D0UWnJbR.js` `hashPlanetFromAltM` + `meshColumnUpM`, `prefs.CniHmaKx.js` `smoothFlights`, and the orchestrator chunk `StylizedTiles.BmVDhHt9.js` (697 KB, behind the 352-byte `GlobeCanvas` wrapper → `GlobeCanvas.CbRaD2Hd.js`) the `"ar-guides"` layer — the DEV seams (`fpvTestSolid`, `fpvClearance`) are stripped in production by design. No `/api` write changed → `verify-live-deletes` not owed. House Chrome closed, no dev server. **The owner tests on his phones next** (T149 the sun-at-the-edge pinch + the mm line · T147 the roll's sign · T150 the EYE readout taste call).
- **2026-09-22 · THE AR SMALL-SCREEN PASS + ROLL OUT + THE AR GUIDES · INSTANT TRANSITIONS + THE CARRIED POSE + THE MESH CLEARANCE · THE /m PLANET BOOT FROM A HASH** (owner: the follow-up to 2026-09-19 after the first phone tests — "be careful not to regress"; `/frame` under investigate-design-v3, implement · Deep; four parallel research agents, every claim cited; leaf `mem:project/wip-2026-09-22-ar-ui-rework-fpv-quirks`). **AR (1) the layout** (`mobile/ArCameraOverlay.tsx` + `styles/mobile/ar-camera.css`): the memo (`AR_CAL_COPY.how`) is gone; the strip is ONE row — `CALIBRATE AR` + `YAW · PITCH · mm` beside it (0.5 rem; the mini-map folds to its 44 px puck while calibrating, `MiniMap` reads `arCam`, so the row runs to the puck — `.m-arcal--on right: 3.9rem`); CONFIRM / RESET / CANCEL are three ROUND 44 px cells (`.m-act--icon`, glyph over label) in a column on the LEFT rail whose bottom sits 12 px above the AIM stick (`11.2rem + 120px + 110px + 12px` — the stick renders 110 px, its 1 px border counts); the 3D ↔ CAM slider stands VERTICAL just above the CAM chip (a rotated range inside a `.m-arcal__mixtrack` box that reserves its length; a SIBLING of the rail seated off `--m-altcol-bottom` + `--m-altcol-h` so the column stays 200 px; its height `--m-armix-h` yields to the mini-map card on a short viewport — 52 px on the twin, 128 on a phone; the ALIGN float clears it). **Owner's rule for every /m surface from here: "we have too little screen space and must use it efficiently"** — recorded in `MOBILE_PLAN.md`'s header. **(2) ROLL LEFT THE CALIBRATION** (`lib/sensors/arCalibration.ts`): `rollDeg` / `rollMaxDeg` gone, `pinchTwist` → `pinchSpread`, the video is rotated by the SENSED roll alone (`arFrame.rollDeg`; the phone-roll sign derives as correct from `poseFromEuler` and is still the device's to confirm — T147); an older `ftw:ar-calib:v1` blob's `rollDeg` is dropped by the sanitizer, no key bump. **The PINCH SIGN was backwards**: `tan(fov'/2) = tan(fov/2) / spread` shrank the feed on a pinch OUT (the only thing the pinch can move on screen is the camera picture — the 3D lens is the FPV focal); now `· spread`, the pinch-to-zoom sense the rest of the app has. **(3) THE AR GUIDES** (`scene/arGuides.ts` NEW, `ARGUIDES`, `lib/sky/screenProject` NEW): every GL guide sits UNDER the `<video>` and loses up to 90 % contrast at the CAM end of the slider — so while AR is on in FPV a DOM layer at z 3 (above the feed and the z 2 labels, under the pad) rings the sun (solid warm ring + core dot), the moon (dashed cool ring) and the tracked target (accent reticle + ticks) with labels, and redraws the sun / moon DAY ARCS as 3-px SVG polylines with a dark halo from the SAME sampled directions the GL arcs use (`dayArcs.arcs()` exposed) through the one pinhole (`projectCameraDir` — unit-pinned against the edge chips' `frameMarker` pair); the layer joins the `body.m.mw-open` PiP hide list (the mapWindowChrome discovery guard caught it; a child label uses `classList` so the guard reads layers, not children). Use case: put the rendered sun on the real one and calibrate on it. **(4) THE DRIFT, diagnosed from code** (research track, T149): calibrated at pitch +5–10°, tilting down slides the aligned building — (H1 ~70 %) the 62° default is too NARROW for a 24–26 mm-eq main camera (a 4:3 sensor spans 69–74° across, the 16:9 video keeps the long side, a stabilisation crop takes ≤ 10 %): the feed is drawn too small and everything off the centre cross slips outward by `(f_v − f)·tan θ` (~28 px at 20° for a 10 % error) → **the default is 68°**, and the guide now says the pinch must be judged with the landmark near the screen EDGE (a scale about the centre cannot be seen at the centre); (H2 ~60 %, the vertical part on a NEAR building) PARALLAX — the eye is a GPS fix + 1.7 m, no rotation fixes metres → calibrate on the sky / a far ridge (the guides); H3 the ~120 ms sensor + look ease (recovers at rest); H4 the compass gate opening on a downward tilt (yaw only, ≤ 0.3°/s post-calibration); H5 distortion. Next on the phones: the sun at the centre, then near the top edge — pinch there, read the mm line. **FPV / MINIMAP (1) INSTANT TRANSITIONS** (`FLIGHT.smoothTransitions` false; `flight.ts` `smooth()` read live per `start`; `stepFovGlide` snaps once per entry / exit / jump via `fovSnapPending`): every `flight.start` — pin arrival, FPV entry / exit, the map window, places, search, MY LOCATION, the re-frame — is the reduced-motion CUT on both shells. The sweep survives behind three doors: the tunable, the chip-less pref `smoothFlights` in `ftw:view-prefs:v1` (`lib/globe/smoothFlightsBoot`, the `debugHud` posture, a plain read with no `rearmed`), and the DEV seam `__globe.smoothFlights(true)` — which the five harnesses that MEASURE a descent (`verify-visual-sweep`, `probe-cpu-profile`, `probe-load-phase2`, `verify-pin-reframe`, `verify-rendering-charter`) now flip before `requestFly`. EXPLORE's cruise is its own model. **(2) THE POSE IS KEPT** — `requestFpvJump` takes a PARTIAL pose (`FpvJumpRequest`; a place / share brings all four fields): the map window outside FPV, MY LOCATION and the /m ▲ 3D hold send lat/lon only, and the entry fills eye · pitch · lens from `fpvCarry` (what the viewer last stood at, recorded at exit), then the planned view's heading / lens, then the defaults on the very first entry (`SceneActions`' `lastFpvFovDeg` retired); standing in FPV the map window's point is a PIN MOVE on both shells (the pin-change re-seat carries the live look / eye / lens rigidly — the /m rule, now the desktop's too); EXIT restores `preFpvOrbit` — the altitude above ground, tilt, lens and /m map mode captured on a fresh entry — over where the viewer now stands (the pin + the walk), approaching along the FPV heading; 200 m / 80° / 38° and the /m 600 m nadir only for a session that booted straight into FPV (trap: the exit branch nulls `fpvKind` first — the carry reads `leavingKind`). **(3) THE MESH CLEARANCE** (`lib/globe/fpvClearance.ts` NEW — pure, 14 tests; `stepFpvMeshFloor`; `FPV.meshClearance` + `meshColumn*` + `meshStepM` + `meshFloorEaseTauMs`; `__globe.fpvClearance(on?)` returns the counters): temp-pin FPV casts ONE vertical column per frame while moving (a down ray AND an up ray — a one-sided material's tops and undersides are both seen) against the building tiles, the enriched cells and the user models — never the terrain — classifies each crossing by its face's OWN normal (`face.normal` → world; `intersection.normal` is flipped toward the ray and cannot tell a top from an underside), folds the crossings into solids top-down (a top opens one, the next underside closes it, no underside = it stands on the ground). The BODY is the 1.7 m under the eye: inside a solid → the feet onto its top the same frame at the standing height (a snap by design; a rooftop structure is the next frame's column — layer by layer, the owner's "next available layer"); on a mesh top the eye follows what is under the feet (off a roof edge = the lower roof or the ground, eased); a pin dropped inside a building lands on its roof at entry; a bridge deck is walked under, a parked car climbed. Verified on the desktop twin with a 12 m test solid (`__globe.fpvTestSolid`, DEV): the lift 287 ms into the walk, floor 81.15 m = the top, eye 1.70 m, the column 6.4 ms once (the tile tree's warm-up), the walk-off drop after 1.8 s, the pin inside lands on top, the kill switch releases. RC10's u2 ring attributes a lift during walk input to the walk. **(4) THE /m PLANET BOOT** (`MOBILE2D.hashPlanetFromAltM` 1,000 km): the 2026-09-18 whole-planet boot read only on a BARE `/m` — but the desktop's Mobile / OPEN /M links hand over the desktop's live pose as `#p=…,1100000,…` (its boot LEO), the UA redirect keeps the hash, and the /m mirror re-writes it every 1.6 s, so the owner kept meeting 1,100 km; an orbital hash now keeps its focus and boots the planet (nadir, the chart), a real place hash below the threshold is honoured (leg 1′: the LEO hash → `18049 KM`, a 3 km hash → `3.0 KM`). **Verification.** vitest **3,276 / 221 files** (was 3,239 / 218: +37, +3) · astro **0/0/12** · knip: the four `normalizeModel` exports it lists are pre-existing, none mine · tsc clean · browser (dev, house Chrome): NEW `verify-fpv-carry-2026-09-22` **29/29** · `verify-ar-calibration` **66/66** (was 52: the vertical slider, the guides layer, the one-row strip, the folded mini-map, the round column, all on RENDERED geometry — three of its first-run misses were the twin's: the slider's padding brushed the mini-map by 2 px → `box-sizing: border-box` + a 52 px floor; "one row" judged by box tops (two spans on one row have different tops) → the mid-line test; the stick's 1 px border) · `verify-mobile-batch-2026-09-08` **127/127** · `verify-mobile-fixes-2026-09-18` **ALL PASS** (+ leg 1′) · `verify-ar-look` **39/39**. NOT released — the owner did not ask for a deploy. **Owner questions, none blocking:** T150 (EYE reads the encoder's 1.7 m on a roof — should it read 13.7 m above the terrain?) · T149 (the drift's device half — the sun-at-the-edge pinch, the mm line) · the roll's sign on hardware (T147).
