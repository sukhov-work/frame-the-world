...
### 2026-09-07i — OWNER VERDICT ON THE PIXEL + BACK TO THE MAIN PLAN (no code this line). The owner tested BOTH mobile features on the Pixel 6 Pro — the heatmap on `/m` and the AR look-around sensors: **"looks and works well."** That is the Android device tier for both (T117's rung 1 — `deviceorientationabsolute` on Chrome — is VERIFIED by the owner's hands; BEST SPOT's Pixel cross-check likewise). **The iPhone's real-world feedback will be provided later** by the owner — it is an open EAR, not a blocker: T117's iOS premises (the compass-tilt gate `FPV.arCompassMinTopHoriz` 0.35, the 25° accuracy ceiling, drift, the feel, the denied copy) stay marked UNVERIFIED-on-device until that feedback lands, and the next session acts on it when it comes, never chases it. **The 2026-09-07g park is LIFTED:** the next session proceeds with the MAIN PLAN — the T77 lane in the order the g-order parked it: lever 11 (the vector-tile parse off the main thread — `ringsOfFeature` + `parseVectorTile` + `tileLocalToLonLat`, 199–262 ms of the Pixel's descent hitch frames, MEASUREMENTS §25.1/§25.5; the parser is already a pure export and `bestSpotWorker` already runs it in a worker) → T115 (the tree-instance locate) → the tails T93 / T102 / T103 / T114-ESRI → the owner calls in ONE batch (the two lean load budgets, `compileAsync`, the terrain BVH, `DAYARC.skylineBehindAlpha`, the flip bank, T113, T118, T116's Pixel FPV re-read) → the three stale remote ship branches. Recorded in `NEXT_SESSION_PROMPT.md` (rewritten around the main plan), `mem:core` Next step, backlog T117 (Pixel VERIFIED, iPhone pending-from-owner).

### 2026-09-07h — THE TWO MOBILE FEATURES LANDED (owner order 2026-09-07g; implement, Deep; `/frame` under investigate-design-v3): **BEST SPOT on `/m`** (the fifth tab, a mobile sheet, no ULTRA — iPhone 17 Pro: finest rung in 6 s, 60 fps with the sheet up, 6–7 ms CPU) · **AR LOOK-AROUND in mobile FPV** (a cited four-rung ladder, an in-house WMM2025, 39/39 on the phone twin; the farm iPhone cannot pass the permission sheet under Appium — the owner's phone is the device tier) · **T116 FOUND + FIXED** — a T77 regression (the tree seats' float32 landing loop) had silently broken BEST SPOT's streaming re-solve on both shells. **1 · BEST SPOT on `/m`.** The fifth `/m` bottom-row item `◎ SPOT` right of SEARCH (`mobile/TabBar.tsx`; five labels ≤ 6 chars fit a 375 px row, harness-measured 69/69 px per cell, no wrap) opens `mobile/BestSpotSheet.tsx` in the FIND idiom — ALWAYS mounted, `bestSpot.open` sticky, collapsing the sheet leaves the field + eight markers on the map; `setOpen` is never called on collapse (it resets the arming switch, right for the desktop's segmented toggle, wrong for a tab). The desktop behaviour WITHOUT ULTRA (no 1 m tier, no per-cell REFINE — `fences.test.ts` pins that `components/mobile/**` never names them, the desktop panel is the positive control) in the `/m` sheet grammar: the arming switch (wide, the feature's real gate), a phone-only `◎ CENTRE HERE` (the temp pin at the map focus — one centre source, R2 unchanged), the four event pills with the day-arc tone by name, the five radius pills, the shared `controls/InstrumentSlider` at touch scale, the R6 lift chip, both legends, the honesty ladder with three lines inline + `▸ 5 MORE CAVEATS` (warnings never fold), the ranked rows (tap = select → GO / LOOK; GO leaves a preview FIRST), the marker tap → FPV preview → `✕ EXIT VIEW` ends it through the shared watcher and restores the borrowed pin. **The copy is ONE copy:** every status line, label and readout moved VERBATIM from the desktop panel into `controls/bestSpotCopy.ts` (`bestSpotStatusEntries` keyed so the phone folds by key, never by index); the desktop panel imports it back (byte-identical behaviour, 47/47). **Engine:** the desktop-only gate is GONE — `bestSpotAllowed` is TRUE on both shells (kept as the one named constant so a future rung has a home; the fence now pins that no shell term creeps back), the sheet's `mobileShell` ctx term retired (§6.10 (C)), and **an ARMED disc keeps the building tilesets + user models attached in either `/m` map mode** (`bestSpotArmed()` in both `shellOn` composites — the solver flattens those meshes; without it the phone reads "NO BUILDING GEOMETRY" forever). **T116 — the regression the mobile harness caught:** the disc's ladder stopped after the 24 m rung and never re-solved. `__globe.bestSpot().stream` (new debug) showed `seatEpoch` +60/s at a settled pose; a new per-reason ledger `seatSettle().enriched.applyReasons` named it `treeWrite: 1` every pass: `t.appliedM` is a `Float32Array`, the float64 target `seatM − cell.seatM` is not representable, `seatLand` returned the exact target, the array stored its float32 neighbour, `next !== applied` was true again next frame by ~1e-7 m — one tree set per cell uploading its whole instance matrix to the GPU EVERY FRAME and the seat epoch never quiet, so the streaming debounce's 90 quiet frames never accrued (a T77 C-1 regression since 2026-09-06k; invisible on the desktop because its first solve usually lands after the tiles). Fix `seatLandF32` (`lib/globe/enrichedMask.ts`) — landed values rounded to the array's precision, "off target" = residual ≥ snap; `enrichedMask.test.ts` reproduces the loop (300/300 writes) and pins the fix (≤ 70). After it the seat epoch is flat and the disc heals: job 2 lands 3 m / 40,401 cells / reach 410 m / 8 spots. **2 · AR LOOK-AROUND** — research first (three cited agents: WebKit + Chromium source, the W3C specs, Apple/Android docs, NOAA/BGS): (a) iOS `requestPermission()` must run SYNCHRONOUSLY inside the tap (WebKit checks `processingUserGesture`, not a window; a rejected promise = no gesture, a resolved "denied" = the sheet's Cancel or an insecure context), the decision is cached per top origin for the Safari process across tabs, and **iOS 13+ has NO Settings toggle any more** (the brief's "Settings → Safari → Motion & Orientation Access" is the removed iOS 12 switch) — the denied copy says quit-and-reopen Safari / Clear History and Website Data; (b) iOS α is RELATIVE (Core Motion `xArbitraryZVertical`, drifts) and `webkitCompassHeading` is the MAGNETIC heading of the device's TOP EDGE (Core Location's default `headingOrientation`), `absolute` undefined, no `deviceorientationabsolute`, 60 Hz with no dedup, WebKit's Euler extraction flips at vertical; (c) Chromium's `deviceorientationabsolute` α is MAGNETIC north (Android `TYPE_ROTATION_VECTOR`, no declination anywhere in the chain), plain `deviceorientation` is relative since Chrome 50, `requestPermission` exists in Chrome 151+ and resolves without a prompt; (d) every compass is magnetic and Dnipro's declination is +8.58° E (2026-09-07; NOAA and BGS agree to 0.001°) — an uncorrected compass is 8.6° wrong. **Built:** `lib/geo/wmm.ts` — an in-house WMM2025 (the 90 public-domain Gauss rows embedded, the NOAA algorithm; pinned to the official 100-row test-value file to 0.01° in D, I and H, plus six sea-level anchors) — the recorded dependency decision: no npm package (`geomagnetism@0.2.0` is CommonJS with four bundled models, `geomag` is still WMM2020, `wmm` is a squat); `lib/sensors/deviceOrientation.ts` — the rear camera's look vector from `R = Rz(α)Rx(β)Ry(γ)` (spec eq. 13a), heading needs NO screen-angle correction (only roll does — computed, NOT applied: the FPV camera has no roll seam), never through the Euler angles themselves, pinned by the named poses (flat/north → 0/−90; upright/east → 90/0; both landscape holds; the spec's own §A.1 formula over a 1,000-point lattice; the α+180/180−β/γ+180 twin), plus `LookSmoother` (a vector EMA + a hysteresis dead-band, seam-free at 0/360); `lib/sensors/orientationLadder.ts` — the four rungs as a pure state machine: `android-absolute` (yaw + declination) · `ios-compass` (the relative frame differs from magnetic ENU by ONE yaw offset, learned as `compassHeading − topHeadingRel` whenever the compass is trusted (accuracy ≤ 25°) AND the top edge has a ground heading (`topHoriz ≥ 0.35`, tilt ≤ ~70°) — held upright the offset holds and the gyro carries, EMA re-absorbs drift on the next dip) · `relative-aligned` (one-tap ALIGN) · `relative-unaligned` (seeded to the camera's heading at arming so nothing jumps); `scene/arLook.ts` — the listeners live in the ENGINE (attach while `arLook && fpvActive`, detach on exit/dispose), the aim is a closure value read per frame (never a 60 fps store write), the mirror writer PUSHED in by the orchestrator (`fences` "mirror-never-seats" holds), the WMM re-evaluated when the eye moves 0.05°; the orchestrator's aim ladder is `arAim ?? skyTrackAim ?? skyLook` on `FPV.arLookEaseTauMs` 70 ms, and **while the aim is live the canvas look-drag and the aim stick's heading stand down** (the phone IS the look; the pinch-FOV and WALK stay); `store/camera` `arLook` (per-session, NOT persisted — a persisted "on" could never re-ask), `arAlignEpoch`, `arLookState` (HUD cadence); `mobile/FpvControls.tsx` `ArLookToggle` — a 🧭 AR round chip above the AIM stick, the rung line beside it (COMPASS · TRUE NORTH (+8.6° DECLINATION) / GYRO ONLY — TAP ALIGN / NO SENSOR DATA), the `⌖ ALIGN` chip on the relative rungs. **Verification.** vitest 2,917/186 (+62/+5: `wmm`, `deviceOrientation`, `orientationLadder`, `mobileBestSpotSheet`, `mobileArLook`; `fences` rewritten) · `astro check` 0/0/11 · knip 0 · **browser (house Chrome, lean twin):** `verify-bestspot-mobile.mjs` **60/60** (the 375 px fit, the sheet, the solve to 3 m, buildings resident on the 2D map, the field surviving the collapse, marker preview + EXIT VIEW, disarm; frame dt p50 41 ms at 4× throttle, heap 109 MB) · `verify-ar-look.mjs` **39/39** (every rung end-to-end through the real listeners with synthetic samples — Chrome 152's `requestPermission` resolved granted from the chip's tap; the CDP virtual absolute-orientation sensor produced no page events headless) · `verify-bestspot.mjs` desktop **96/101 by design** (the D8 four) · the post sweep vs `post2-2026-09-07f`: **draw-count gate 13/13** (calls/tris identical; the Everest sunset's 2 % tri drift is streaming), pixel diffs the boot-to-boot band, `/m` differs by the fifth tab. **Device (Device Farm iPhone 17 Pro, iOS 26.3.1, `devicefarm-mobile-h-2026-09-07T19-01-41.json`, ~6 device minutes):** BEST SPOT — boot 5.9 s, the disc armed on the 2D map: first ink 71 ms, rungs 24/12/6/3 m in 70/75/132/397 ms of worker time, **finest rung 6.1 s after arming** (the refusal + heal), 18,887 of 40,401 scored, reach 410 m, 1 + 4 building meshes, sheet up; the 95 s hold at **60 fps flat (dt 17/17/17–21 ms), CPU 6/7 ms p50/p95, LRU 3/78/2 MB, 11 hitches all at the load, page alive at 107 s** — THE GATE PASSES; a marker preview entered FPV with the shortlist intact and the sheet came back after exit. AR — the Appium `element.click()` is NOT a WebKit user gesture: `requestPermission()` rejected, the chip said TAP AGAIN (the right copy), and without the grant iOS sent zero events — the device tier for AR is the owner's own iPhone (T117). **Open:** T117 (AR on a real iPhone: the compass-tilt gate, the accuracy range, drift) · T118 (the first `/m` solve is a refusal until the buildings stream — 6 s on the phone, 11 s on the twin; owner call) · T116's Pixel FPV re-read (one fewer per-frame instance upload). Files: `controls/bestSpotCopy.ts` (new) · `mobile/BestSpotSheet.tsx` (new) · `mobile/TabBar.tsx` · `mobile/MobileShell.tsx` · `panels/BestSpotPanel.tsx` · `styles/mobile/chrome.css` · `styles/mobile/fpv.css` · `globe/StylizedTiles.ts` · `scene/bestSpotSheet.ts` · `scene/bestSpotFeed.ts` · `scene/enrichedBuildings.ts` · `scene/arLook.ts` (new) · `lib/globe/enrichedMask.ts` · `lib/geo/wmm.ts` (new) · `lib/sensors/{deviceOrientation,orientationLadder}.ts` (new) · `store/camera.ts` · `globe/tuning.ts` FPV `ar*` · `mobile/FpvControls.tsx` · scripts `verify-bestspot-mobile.mjs`, `verify-ar-look.mjs`, `probe-bestspot-mobile-{ladder,seat}.mjs`, `tools/devicefarm/ios-baseline.mjs --legs bestspot,ar` · tests as named · `bestspot/README.md` banner · `MOBILE_PLAN.md` status · backlog T116–T118 · `mem:project/wip-2026-09-07-mobile-bestspot-ar`.

### 2026-09-07g — OWNER ORDER: two MOBILE features come first, the T77 lane and every open call PARK behind them (order given at the end of session f; no code this line). **1 · BEST SPOT on `/m`:** the heatmap becomes the FIFTH base item of the `/m` bottom row, right of SEARCH (`components/mobile/TabBar.tsx` — today SCENE · PLAN · FIND · SEARCH); the desktop's behaviour WITHOUT the ULTRA options (the 1 m re-solve and its fields), in a nicer mobile-adapted sheet, not a port of the 928-line desktop panel; the store, the scene sheet and the worker are shared (`store/bestSpot.ts`, `scene/bestSpotSheet.ts`, `lib/geo/bestSpotWorker.ts`); **performance on the iPhone is the gate** (Device Farm 17 Pro under the T83 caps and the 2 GB jetsam line; the Pixel much less critical). This un-parks BEST SPOT for its mobile surface only (`bestspot/README.md` stays the entry point; `verify-bestspot` stays 96/101 by design). **2 · AR look-around in mobile FPV:** only in mobile FPV, a small toggleable button with a meaningful icon on top of the aiming-joystick UI (`components/mobile/FpvControls.tsx`); when ON the phone's physical orientation aims the virtual camera — compass + accelerometer + every sensor the device allows, permissions requested with the best user flow (the toggle's tap is the gesture; never at load), a DEGRADATION LADDER for the most accuracy on modern iPhones / iOS (iOS `DeviceOrientationEvent.requestPermission` + `webkitCompassHeading` with the magnetic → true correction · Android `deviceorientationabsolute` / `AbsoluteOrientationSensor` · relative gyro with a one-tap align · none) — every rung to be VERIFIED in research before design; the orientation math a three-free unit-tested module; the desktop harness drives `deviceorientation` through CDP, a real iPhone verifies the permission flow and the compass. **3 · PARKED until both land:** lever 11, T115, the tails (T93, T102, T103, T114's ESRI half), the two August remote ship branches, and every owner call (the lean load budgets, `compileAsync`, the terrain BVH, `DAYARC.skylineBehindAlpha`, the flip bank, T113). Recorded in `NEXT_SESSION_PROMPT.md` (rewritten around the order), `mem:core` Next step, `mem:project/wip-2026-09-07-t106-pixel-read` §Owner order.

### 2026-09-07f — T106 READ ON THE PIXEL (a+b hold: dt p95 316.7 → 50.0 ms, hitch main thread 6.73 → 3.93 s) · the OSM handler had been on three's SLOW path (interleaved b3dm positions) — fast path fixed + the OSM handler made TWO-PHASE on its own queue · the GC behind the "13.5 ms step": the edge `Map` → a typed table, both builders POOLED per queue (Pixel worst drain enriched 13.5 → 6.7 ms, OSM 410 ms atomic → 3.1) · T114 counted · the eight stale worktrees + nine `claude/*` branches retired (backup refs) · two phone traps (thermal status, the keyguard) (implement, Deep; `/frame` under investigate-design-v3; owner order 2026-09-07f: proceed with the brief, the Pixel available; `mem:project/wip-2026-09-07-t106-pixel-read`). **Boot:** ship log clean (PR #111 landed, master `7d5d925`), VPN IE, ion 401 (from the phone's page too — the phone read country UA yet ion answered 401, so the T98 gate passed), house Chrome :9333, `wix dev` with `.vite` aside, pre sweep `pre-2026-09-07f` 12/14 (`legacy-m` T103 + `legacy-fpv-eye` Δ1, both the frozen-frame check, as `pre-2026-09-07d`), sheets read. **1 · The Pixel read of T106 (MEASUREMENTS §25.1).** `probe-cpu-profile --leg descent --device` vs §21.3: dt p95 **316.7 → 50.0**, max 848 → 400, hitch frames' main thread **6.73 → 3.93 s**, `enrichedBuildings.ts` `load-model` GONE from the callers (phase 1 max 0.6 ms; the units drain budgeted); the biggest app item left was the OSM `buildings.ts` handler — **410 ms in the hitches through three's `EdgesGeometry` (361 ms self)**. `probe-load-phase2 --device` (NEW flag: attaches to the phone's tab, no emulation, asserts `gnd > 0`): 70 units, worst drain **13.5 ms** at budget 3 — one builder STEP, not chunk work. **2 · The OSM slow path (§25.2).** The phone's resident OSM meshes: 11/11 `Float32Array/interleaved` (position · normal · `_batchid`, stride 8 — three's GLTFLoader on Cesium's b3dm), and `edgesGeometry.ts`'s gate demanded a PLAIN attribute — every OSM tile had taken three's path since slice (d), desktop too; the doc comment was an assumption and no OSM counter existed. `positionsF32` now de-interleaves (the same floats three's `toNonIndexed` reads; `test/components/globe/edgesGeometry.test.ts` pins identity against a real `InterleavedBuffer` with padding noise, and that a normalized Int16 attribute still takes three's path). **3 · The OSM handler two-phase (§25.2).** `scene/buildings.ts`: PHASE 1 synchronous (material swap, F1 fill birth, tone seed, shadow flags; `handlerMaxMs` 0.1–0.4 on the phone) + one edge unit per mesh on its OWN `loadQueue`, drained in `update()` after `tiles.update()` under `BUILDINGS.loadBudgetMs` **3** / `loadBudgetMsLean` **1.5** (a separate budget — the two queues drain in the same frame, OSM then enriched, so a frame spends up to their sum; keyed on `lean` by the orchestrator), nearest tile first (`lookBiasedDistance` on `makeTileCenterReader`'s sphere centre), the strokes born with their own `frameNow()`, `dispose-model` cancels by scene, `loadQueue.clear()` in dispose. Seams `__globe.buildingsLoad()` (ledger incl. `allocTop` = the three costliest constructions with verts/tris/nth, `scratchReuses/Growths`) · `buildingsLoadBudget(ms)` · DBG `buildings.osmLoadPending` / `osmLoadMaxMs` (the two `buildings.load*` rows relabelled "enriched"). **4 · The GC behind the steps (§25.3, six Pixel runs).** With the OSM two-phase the OSM worst drain read 3.2, then 16.7 the next run with `allocMaxMs` 15.5; the enriched stayed 12.5–13.4 with `allocMax` 1.5 — and `allocTop` read 19.3 ms for a 43k-vertex mesh vs 1.3 for a 31k one in the same run: V8's external-memory GC landing in whichever step allocated (each unit: a `Map` of up to 3 × tris — ~10 MB of heap entries per big cell — plus ~9 MB of typed arrays; ~600 MB of churn per descent). Built, pure, identity-pinned: the edge map is an open-addressed typed table (`hK0/hK1/hSlot`, the keyer's mixer; `slotOf` and the `ua·N+ub` key gone); `createFastEdgesScratch` / `createAttributorScratch` — ONE scratch per queue (a queue steps one unit at a time), tables grown to the biggest mesh seen and reset over the build's own range (`fastEdges.test` +3: falling/rising sizes ≡ fresh, stale entries never leak, the attributor likewise). Pixel: enriched worst drain **13.5 → 6.7** (edgesMax 4.0, maskMax 6.7, allocMax 2.3; reuses 86 / growths 14), OSM **→ 3.1** (allocMax 2.4; 11 / 4); `enrichedBench(50)` 0/0 every run. Desktop `high`: enriched 8.5 → **7.0**, OSM 3.3, frames p95 24.4, >33 ms 11, identity 0/0; lean twin: enriched 9.2, OSM 3.3, p95 50.8 → **42.8**, max 255 → 174. The enriched ledger gained `allocMs/allocMaxMs` and `locateCalls/locateMs/locateMaxMs/locateFeatures/locateMaxFeatures` (the one-shot `ensureLocated`, the parts-first order — TIMED, not yet sliced). **5 · Two phone traps (§25.4, README §B).** The ninth back-to-back descent read dt p50 33 / max 1,897 ms with `dumpsys thermalservice` at **status 4** (CPU 72/80 °C, big cores 984 MHz) — VOID; the recipe now reads the status before every timed run and waits ≤ 1. Cooling it with `KEYCODE_SLEEP` LOCKED the phone (secure keyguard, `wm dismiss-keyguard` refused; the tab `hidden`, rAF 0) — cool with the tab on `about:blank`, screen ON. **The like-for-like post profile, taken once the owner unlocked the phone at thermal 0 (§25.5):** hitch frames 66 → **55**, their main thread 3.93 → **2.78 s**, max frame 400 → 283, `buildEdgesGeometry` / `EdgesGeometry` gone from the callers and the self-time table (361 → 0), app in the hitches 1,304 → 691 (the vector tiles 199 now lead — lever 11), gc unchanged at 178 over the leg; `ensureLocated` fired 58× with 0 FEATURES (the parts-first order never occurred; its 7.5 ms max is the tree-instance loop — **T115**); `terrain.memo.regionless` 0 over 64 invalidations (T114's CWT half read). **6 · T114 (half).** `HeightMemoStats.regionless` counts the region-less early return; DBG `terrain.memo.regionless` (warn > 0); `heightMemo.test` asserts 3 then holds at 3 on a real region. The browser half = reading 0 on that row after a descent. **7 · The stale worktrees.** All eight `../ftw-wt-*` dirty diffs checked against master (`git apply --check -R`: LANDED for the slice files; the T96 fence lives on master in a later form; `UNCLEAR` only where master's context moved) → each worktree's dirt SNAPSHOTTED to `refs/backups/wt-2026-09-07f-<name>`, the worktrees removed, the nine local `claude/*` branches deleted; `git fetch --prune`; two August remote ship branches remain for the owner (`claude/ship-20260814-003854` PR #35 closed, `claude/ship-20260822-002057` no PR — both superseded). **8 · `compileAsync` (slice 0's rider):** evidence on record — desktop 0.9 % of the leg, the Pixel 68 ms / 0.7 % (312 ms only on the cold, throttled run); no per-material attribution exists; NOT built, owner call (close like lever 10, or a first-load-only prewarm). **9 · THE LATE-CHILD TRAP (§25.6) — a regression caught by the post sweep's DRAW CALLS, not its pixels:** same triangles per pose, fewer calls on every pose (zoom-sweep 437 → 400); at rest HEAD drew 220,721 line segments, the working tree 38,087, with the same 41 OSM edge objects present — all 41 at the IDENTITY world matrix (`TilesRenderer.setTileVisible` updates a tile scene ONCE; `TilesGroup.updateMatrixWorld` recurses only when the group's own matrix changed; a child added later is never reached, renders at ECEF (0,0,0) and is culled). The enriched deferral of 07e had the same exposure, hidden by the seat passes. Fix: every deferred stroke calls `updateMatrixWorld(true)` after `c.add(edges)` (both handlers) — 437 / 220,721 again, 41/41 + 11/11 seated. Fences: `fences.test` pins the call; `verify-visual-sweep --compare` gained a DRAW-COUNT GATE (triangles within 0.5 % of the golden and calls down > 3 and > 2 % ⇒ FAIL, from the golden run's `report.json`). **Gates:** vitest **2,855/2,855** (181 files; +10) · `astro check` 0/0/11 · knip 0 · pre sweep `pre-2026-09-07f` 12/14 · post2 sweep `post2-2026-09-07f` 12/14 (`legacy-fpv-eye` + `legacy-city` Δ1 — the T103 family; `legacy-m` passed this time), the NEW draw-count gate PASS on all 14 (calls identical to the golden on 12, ±2 with ±22 tris on the other two = streaming), the pixel diffs the §24.3 noise band (`legacy-m` 45 % = the imagery composite + the live clock chip "−17d 1h" + the dial's day-arc ring, same scene); the first post run (`post-2026-09-07f`) is VOID — it is the run that caught the late-child trap. Records: MEASUREMENTS §25 · backlog T106 (Pixel read + the OSM fix), T114 half · README §B traps · NEXT_SESSION_PROMPT refreshed · `mem:core` status.
