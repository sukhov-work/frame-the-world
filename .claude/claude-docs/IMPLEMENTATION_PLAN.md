...
| **T77 — architecture + performance audit and revamp** of rendering and scene management | bundle entry `rendering/README.md`; plan `rendering/T77_AUDIT_PLAN_2026-09-05.md` | ◐ **OPEN** — re-dated 2026-09-17 (audit #4 D5; the row had frozen at 2026-09-06). MEASURE + both phones 2026-09-06; slice 0 / T79 closed 2026-09-06e; T80 CLOSED 2026-09-06n (fused bloom, dt 14.2 ≤ 15); slices A/B shipped 2026-09-06h/k; levers 8 (terrain BVH, 2026-09-08d) and 11 (vector-tile worker, 2026-09-07j) built and read on the Pixel; T106/T107/T115/T116 fixed; T123 fixed to the owner's gate 2026-09-10 (levers a + d). Still owed: `applyFeatureSeats` 461 ms, the ephemeris in the hitch frames, the ~900 mesh-raycast calls per frame, the farm FEATURE legs (DECISIONS 2026-09-10; `NEXT_SESSION_PROMPT.md`) |
| **AR on `/m`** — look-around (2026-09-07h) · the GYRO-LED ladder · CAM, the camera view · visual CALIBRATION (2026-09-19) | `ARCHITECTURE.md` §7f; `MOBILE_PLAN.md` status; DECISIONS 2026-09-07h, 2026-09-19 | ◐ built + twin-verified (`verify-ar-look` 39 · `verify-ar-calibration` 52); **the DEVICE pass is owed — backlog T147** (the owner's Pixel 6 Pro + iPhone over HTTPS) |
| **Owner batch 2026-09-19** — ▣ ADD A BOX primitive · lift rails 300 m · the DELETE wire (`jsonWriteInit`) · the /m white-ground fix | `ARCHITECTURE.md` §6 + §7d addendum; `MESH_SUITE_PLAN.md` addendum; DECISIONS 2026-09-19 | ☑ built + browser-verified; open tails **T146** (seamless overlay handover — desktop boot-`mid`) · **T148** (the box on `/m`? owner ruling) |
| **Owner batch 2026-09-22** — the AR small-screen layout · roll out of the calibration · the pinch sign · the 68° default · the AR GUIDES above the feed · INSTANT transitions (hidden toggle) · the CARRIED pose (entry / exit / jumps) · the MESH CLEARANCE · the /m planet boot from a hash | `ARCHITECTURE.md` §7a′ + §7f; `MOBILE_PLAN.md` header; `conventions/globe-tuning.md` §2026-09-22; DECISIONS 2026-09-22 | ☑ built + twin-verified (`verify-fpv-carry-2026-09-22` 29 · `verify-ar-calibration` 66 · `verify-mobile-batch` 127 · `verify-mobile-fixes` ALL PASS incl. leg 1′ · `verify-ar-look` 39); open tails **T149** (the drift's device half) · **T150** (the EYE readout on a mesh floor) · **T151** (the clearance's known edges) |
