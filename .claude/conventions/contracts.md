...
## 2. localStorage keys (all `ftw:*`; re-diffed 2026-08-22, audit #3 D7; re-diffed 2026-09-17 against `grep -rn '"ftw:' src` — still these six on 2026-09-17 — SEVEN since 2026-09-19 (`ftw:ar-calib:v1`); the T126 edit journal (`store/editJournal.ts`, `lib/edit/editJournal.ts`) is memory-only and persists nothing)
...
| `ftw:ar-calib:v1` | `src/lib/sensors/arCalibration.ts` → `store/camera.arCalibration` | new 2026-09-19 (the /m AR visual calibration): `{ yawDeg (a compass BIAS — lives inside the engine's yaw trim), pitchDeg, camLongFovDeg, savedAtMs }`; every read sanitizes onto rails; CONFIRM replaces it, RESET removes the key. **2026-09-22: `rollDeg` LEFT the record** (roll is not calibrated; an older blob's field is dropped by the sanitizer — no key bump, every field is defaulted). A SEPARATE key on purpose: `mobileArLook.test` pins that `prefs.ts` never names AR |
