...
| `ftw:ar-calib:v1` | `src/lib/sensors/arCalibration.ts` → `store/camera.arCalibration` | new 2026-09-19 (the /m AR visual calibration): `{ yawDeg (a compass BIAS — lives inside the engine's yaw trim), pitchDeg, rollDeg, camLongFovDeg, savedAtMs }`; every read sanitizes onto rails; CONFIRM replaces it, RESET removes the key. A SEPARATE key on purpose: `mobileArLook.test` pins that `prefs.ts` never names AR |
