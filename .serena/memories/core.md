...
UX batch is mapped into sessions S1–S7 in `.claude/claude-docs/archive/PHASE_5_5_UX_BATCH.md` (canonical;
...
**App working title = SIDERA (owner 2026-08-14; UI-facing only — repo + technical ids stay "frame the world"). Next step *(after the 2026-08-15c owner UX batch ×9 — context-menu quick toggles + camera-keeping rise/set + moon % · TRACKING camera lock · /m FIND 4th tab + STICKY standings (Pixel fix) · mobile-default entry · /m SAVED PLACES + quota dropped · altcol lift · collapsible /m mini-map; `mem:project/wip-2026-08-15-uxbatch2`; fold the new surfaces into the GUIDE inventory)*: **G1 of the IN-APP USER GUIDE (owner order 2026-08-15 — dedicated multi-session track, plan = `.claude/claude-docs/GUIDE_PLAN.md`**: precise feature inventory checklist + hybrid UX shape (GUIDE panel both shells from ONE guideContent.ts data module + 3 coach-mark journeys) + G1/G2/G3 breakdown; FAQ host/absorb decision in G1; in-browser inventory audit BEFORE copy). Phase 8c / M4 / §3.5-V2 queue behind it. UX BATCH ×5 SHIPPED 2026-08-15b both shells (`mem:project/wip-2026-08-15-ux-batch`): PLAN/FIND = ONE topnav segmented toggle (`PlanFindToggle`; FIND → "FIND IN FRAME") + ONE shared draggable+RESIZABLE window (both panels mount at one spot under the "planfind" drag/resize key; NEW `usePanelResize`/`ResizeGrip` in ui/DragGrip — border-box --win-w/--win-h contract; TRAP: closed panels stay mounted → shared-key session state must be adopted on render, both hooks do) · sky-ghost first-gap root-caused (`GHOSTS.nowGapDiscs` 1.2→0.25 — the old cone ate k=±1…3 at fine steps; maxPerSide 8→15 = 30 discs, 4-site clamp sync; skyGhosts.test.ts drives the real InstancedMesh headlessly) · daytime nav scrim+halo in global.css (island buttons unreachable from scoped CSS) · /m login + MY PLACES + one-tap ◎ SAVE VIEW (`MobileAccount`/`MobilePlaces`/`SavePlaceChip`; pose hash VERIFIED riding the real dev OAuth hop; places ≠ pins-browsing, MOBILE_PLAN clarified) · scrubber pinned split amber-past/blue-future (NEW chrome token `--color-time-future #7cb0f5`, not in the GL bridge). §3.5 SUNSETS-IN-FRAME SHIPPED 2026-08-15 both shells (`lib/ephemeris/sunEventFrame.ts` event-anchored day loop, refracted-labels/airless-geometry PINNED, disc-extent pads, FindPanel SUNSETS section + PlanSheet SunsetSection — `mem:project/wip-2026-08-15-sunsets-in-frame`); M3 COMPLETE + hover-floor fix 2026-08-14 night-6 (`mem:project/wip-2026-08-14-mobile-m3c` · `mem:project/wip-2026-08-14-night6-hover-floor`) (spec WRITTEN in PLANNING_QOL_PLAN §3.5; consider a SUNSETS row inside FindPanel — don't re-bury; the FIND dd.mm label layer from night-5 is a building block — `mem:project/wip-2026-08-14-find-accuracy-labels`). DONE 2026-08-14 night-4b (owner: v2 core "works exactly as i imagined"): FIND markers RESTYLED — hairline quad-gap identity rings (ringWidthN .07 + ringAlphaGain 2.2 — hairlines need alpha compensation) + semi-transparent phase-ACCURATE body pictures (moon LROC phase-lit per hit instant via per-instance sunDir in billboard space; sun limb-darkened; bodyAlpha .6) + per-hit day-arc sky paths in hit colours (sampleDayArc cache per body|day, pathAlphaK .3 anti-scratch-bundle, hover boost ×3) · scrubber in-frame band 5px@78% (base ribbon = absolute above-horizon visibility) · SIDERA in all UI name spots (Layout/Mobile titles, logo, UploadFlow brand, ics summaries). Verify traps: React hover probes need mouseover (mouseenter no-ops) · hash-only goto = SAME-DOCUMENT nav (about:blank between loads) · wix dev port flipped 4322→4321. Shots find-rework2-01..04. DONE 2026-08-14 night-4: FIND v2 OWNER REWORK — dedicated FindPanel island (⌖ FIND pill beside PLAN, mutually exclusive open; frame-as-query: EVERY following day scanned at the EXACT scrubber wall-clock hour vs the LIVE frustum — frameFinder.frameStandings/bodyDayPositions + FIND_VIS visibility model + store/time.sameLocalTimeInstants; bodies sun/moon/GC) + in-frame ghost projections (scene/findGhosts + FINDGHOSTS tuning: hollow per-hit-coloured rings/GC-diamond from lib/theme/findPalette (10 tokens, real-body colours excluded), alpha = date ramp × visibility, click = setTime+setTarget with the camera UNMOVED, hover row↔ghost sync via store/find) + FindCard DELETED from the PLAN deck. TRAP: panel self-compute MUST gate on open&&poseKey — focus-keyed memos churn per frame during flights (an ungated 550-sample pass froze the boot flight). Gates 842/842 (+14) · astro 0 err/5 hints; browser-verified find-rework-01..03 (click-jump exact, zoom 60°→3° drops 45→0 hits live); owner-device tier open (ghost-ring size: FINDGHOSTS.ringRadFrac/minDiscDeg/alphaNear first); mem:project/wip-2026-08-14-find-rework. DONE 2026-08-14 night-3: QoL-4 owner batch — day-moon dark-chunk ROUND 3 (moon depthWrite OFF — the disc's depth wall intermittently rejected the additive dome; readPixels dark-frame detector 20/485 → 0/469; see mem:project/wip-2026-08-14-qol4-batch before touching scene/sky.ts) · reticle = lunar parallax (per-frame topocentric targetDirW from targetPosW; 0.878°→0) · GHOSTS.nowGapDiscs disc-exclusion · hover ring (SKY.hoverRing*) + skyHoverGain 0.32 · sky hover NAMES (lib/sky/hoverNames + scene/skyNames, lazy catalogs, night-gated) · QoL-3 SHIPPED desktop (FindCard az±3/el±0.5 via frameFinder.azElHits ~1 ms/day · MoonCalCard quarters/apsides/★supermoon via lib/ephemeris/moonCalendar · SpotStarsCard NPF via lib/photo/npf with frustum-auto δ · SIZE→DIST TargetPanel row via lib/geo/sizeDistance). Gates 828/828 · astro 0 err; browser-verified qol4-01..04; owner-device tier open. DONE 2026-08-14 evening: QoL-2 desktop COMPLETE + owner 7-ask batch — `lib/ephemeris/frameFinder.ts` (injected-sampler frameCrossings + nearestFrameCentre) · FrameCard/TodayCard (self-computing, per-row .ics via new `lib/export/ics.ts`) · GHOSTS temporal chain (`scene/skyGhosts.ts`, store/sky knobs) · sun/moon first-class targets (`bodyTarget`, `body:*` search rows) · right-click SkyContextMenu (`camera.skyMenu` mirror) · WASD + ⇧␣ descend · daytime-moon dark-disc shader FIX (day-sky alpha + discard + no double tonemap — see mem:project/wip-2026-08-14-qol2-batch §moon before touching scene/sky.ts) · HUD==minimap 210px · `.ct--fpv` compact deck. Gates 784/784 · astro 0 err; browser-verified qol2-01..08; owner-device tier open. DONE 2026-08-14 late: QoL-1 desktop COMPLETE — §3.1.D trace SHIPPED with a recorded deviation (planFeed mirrors skyline BINS `store/plan.profileBins`; the RAIL computes the trace via `targetElevationSeries`+`traceStates` (dayArc) + `azAltFrameMarker` (offscreen) + `FpvHud.aspect`; bins = the QoL-2 frameFinder profileFn seam) + playhead cursor (`.ts-cursor` bar+pennant replaces the knob dot, owner order); gates 763/763 · astro 0/0/5 · browser-verified qol1t-01..03; `mem:project/wip-2026-08-14-qol1-tail-trace`. DONE 2026-08-14 night: PhotoPills DEEP REVIEW (Wayback-sourced; adoption record PLANNING_QOL_PLAN §1 R1–R20) + Phase 8 ladder RE-RULED (8-QoL-1/2/3 → events → tools; DSO slid behind) + **QoL-1 SHIPPED desktop** (scrubber v2 infinite conveyor + full light bands LIGHT_DEG + sun/moon elevation curves + real hour labels + event-step taps · Space=FPV hold-ascend · My-spot→FPV both shells; vitest 752/752, browser-verified qol1-01..08; `mem:project/wip-2026-08-14-qol-batch`). M2 DONE 2026-08-14 (FPV touch: joystick walk seam `fpvWalkInput` + pinch-FOV + ⤒/⤓ nudges + HUD row + wake lock + minimap on /m; pivot invariant re-verified on touch 0.000 m; phone-viewport browser-verified; `mem:project/wip-2026-08-13-m2-fpv-touch`). M1 DONE 2026-08-13 (phone-viewport browser-verified; `mem:project/wip-2026-08-13-m1-mobile-planning`). DONE
...
UPGRADE→paidPlansCheckout redirect (`lib/wix/planUpgrade.ts`) · click-time returnTo w/ #p= hash ·
?purchased=1 toast · UNLIST arm. OWNER ACTIONS PENDING: install Pricing Plans app + one public plan
(site lacks it — APP_NOT_INSTALLED); manual purchase→mark-paid→email loop (Phase-6 DoD). Details:
`mem:project/wip-2026-07-17-phase69-marketplace-batch`. PHASE 6 — MARKETPLACE-LIGHT SHIPPED 2026-07-16 (LATEST; gates vitest 587/587 · astro 0/0 · wix build Complete; runtime-VERIFIED in wix dev + live gateway). Self-serve: a member lists their OWN, already-PUBLIC pin as a **Wix Stores Catalog V3 DIGITAL product** (owner asked for V1 but the site is V3 — the gateway hard-rejects V1 `428 CATALOG_V3_CALLING_CATALOG_V1_API`; V3 is BETTER — a headless app CAN create digital products on V3, V1's attach method is exposure=INTERNAL). The retained `originalFileId` IS the digital file. `POST /api/listings` (elevate → ownedPhoto → `productsV3.createProduct`, digitalFile=originalFileId → writes productId/variantId/price/currency to Photos+PublicPins) · `DELETE` unlist (deletes the product) · `GET` owner-sales (elevate `orders.searchOrders`, PAID). BUY runs CLIENT-SIDE in the buyer identity: `@wix/ecom checkout.createCheckout` + `@wix/redirects createRedirectSession` → the Wix-hosted checkout; manual payment → owner marks paid in the DASHBOARD → the preinstalled Stores automation emails the 30-day link. KEY TRAP: the checkout `catalogReference` needs `options.variantId` (productId alone → EMPTY checkout — verified) + the fixed Wix-Stores appId `215238eb-…` (NOT the TPA id 1380b703). E2E verified: member list → real product created under the member session → checkout resolved the DIGITAL line item @€7.50 → unlist tore it down (`scripts/verify-listing-member.mjs`). Remaining = the real pay→delivery-email loop (owner's manual dashboard step, not code). NEW: `lib/market/listing.ts`·`lib/wix/photosData.ts`·`pages/api/listings.ts`·`store/market.ts`·PhotoDetailPanel MarketSection·MyPins price chip; installed `@wix/ecom`+`@wix/stores`. `mem:project/wip-2026-07-16-phase6-marketplace-research`; DECISIONS "2026-07-16 (LATEST — PHASE 6…)". PRIOR: PRE-PHASE-6 UI/UX BATCH SHIPPED 2026-07-15 — 4 owner tasks, browser-VERIFIED (scripted headless-Chrome CDP; vitest 573/573 · astro 0/0 · wix build): (1) Milky Way = NASA SVS Deep Star Maps 2020 starless haze sphere on the −GAST star group (landmark-verified J2000 dir→RA/Dec mapping; dust lanes/bulge/colors; diffuse layers attenuate below 35° FOV — MILKYWAY.narrowFov*); (2) sun/moon FPV razor-cut root-caused (closest-approach vs WGS84_A sphere + tc-guard = binary cut at 0° geocentric elevation, ~0.3° above the true horizon) → ANGULAR fade vs the exact ellipsoid horizon (CPU float64 horizonTerms/horizonBandSin in scene/sky.ts, street 0.08° ↔ orbit 0.6°), atmosphere sinEl=0 seam C1-fixed (sRel + smoothstep zenith + softabs haze crest, budget guard untouched), stars+haze sink at the same line; (3) SAVED PLACES: SavedPlaces collection LIVE + /api/places + placeRecords.ts (UrlFpvPose+timeMs, one contract with #f=) + MY PINS·PLACES tabs + SAVE PLACE on the camera deck + camera.fpvJumpRequest → exact share-link-path restore incl. FPV→FPV re-jump (member E2E verified; WATCH: transient Wix Data WDE0054 on the fresh collection); (4) PIN chip (camera.pinsVisible → PinsHandle.setVisible gates render+pick in one; FPV hides by default, wasFpvActive-captured restore-on-exit). `mem:project/wip-2026-07-15-prephase6-uiux`; DECISIONS 2026-07-15 batch line. PRIOR: DOCS REORG DONE 2026-07-15 — claude-docs root now holds ONLY ARCHITECTURE.md/DECISIONS.md(+ARCHIVE)/IMPLEMENTATION_PLAN.md/NEXT_SESSION_PROMPT.md/PROJECT_SEED.md; everything else moved (git mv) into `provenance/` (DEEP_RESEARCH, CLAUDE_DESIGN_MEMO) · `dnipro-enrichment/` (DNIPRO_* + OSM2WORLD_EXPERIMENT_PREP) · `rendering/` (RENDERING_QUALITY_PASS) · `archive/` (PHASE_5_5_UX_BATCH, ARCHITECTURE_REVIEW, B19_HANDOFF); references updated repo-wide (37 files incl. these memories; DECISIONS verbatim history deliberately untouched); NEXT_SESSION_PROMPT rewritten as the Phase-6 brief (scope/DoD/pre-reads + catalog-v3-vs-v1 open Q + carried tails); ARCHITECTURE.md + IMPLEMENTATION_PLAN.md refreshed to as-built (5.5 ☑, NEW Interlude 07-12→14 ledger, Phase 6 = NEXT). Gates re-run clean astro 0/0 · vitest 548. `mem:project/wip-2026-07-15-docs-reorg-phase6-prep`; DECISIONS "2026-07-15 (LATEST — DOCS REORG + PHASE 6 RUNWAY)". PRIOR: FPV FOLLOW-UP SHIPPED 2026-07-14 — (a) BUILDINGS-slider solidity fades gradually+uniformly: the 55→56 jump was the binary `depthWrite=k>0.55/0.6` flip → replaced by the F1 screen-door Bayer dissolve (per-building hash jitter → continuous city-wide response; materials OPAQUE + depth-writing at every k; `uFlatAlpha` in buildingMaterial.ts + `uFtwTreeAlpha` on trees; setGhost/setSolidity lose all transparent/depthWrite churn); (b) FPV views shareable: `#f=lat6dp,lon6dp,eyeM,heading,pitch,fov[&t=]` (GROUND-RELATIVE eye by design; write = live camera at pose-hash cadence, `!fpvActive` gate removed; restore = bootPoseAt (the refactored ONE #p/#f boot helper) → temp-FPV entry consumes exact basis/eye/FOV via pendingFpvShare; Welcome+boot-poster skip `/^#(p|f)=/`; photo-FPV shares as temp view). Gates astro 0/0 · vitest 548 (+5) · wix build · browser-VERIFIED (uiux-05..09; round-trip exact, pitch ±0.2° settle; #p + welcome regression-clean). `mem:project/wip-2026-07-14-uiux-qol-batch` §Follow-up; DECISIONS "2026-07-14 (LATEST — FPV FOLLOW-UP)". PRIOR: UI/UX QoL BATCH SHIPPED 2026-07-14 — all 6 owner tasks browser-VERIFIED (astro 0/0 · vitest 543 (+13) · wix build; shots `verify-shots/uiux-01..04`; no globe scene/shader code touched): uniform hover-reveal drag handles OUTSIDE panels top-right (`:hover > .drag-grip`; scrolling panels moved overflow to inner .pd-scroll/.mp-scroll/.pp-scroll wrappers — overflow on a panel root clips the tab) · time PLAYBACK (±H/±M steppers + PLAY/STOP + presets ×1/1min-s/10min-s/1hr-s via SCRUB.playRates; `sceneTimeMs()` derives timeMs+(now−playWallMs)·rate → fluid scene, zero per-frame store writes; play(1) from LIVE no-op; setTime mid-play REBASES; FF = red ▶▶ labels + FAST-FORWARD chip) · precise `<input type=time>` + CUSTOM scene time shareable as `#p=…&t=<utcMs>` (formatSceneHash/parseTimeHash; LIVE never shared; boot restores) · PLAN panel top-left under the title unfolding DOWN (z 31, max-height clears the mini-map slot) · ZERO horizontal scroll (root cause: overflow-y:auto forces x to auto + tips ::after pills laid out while hidden → overflow-x:hidden everywhere + poking tips re-anchored; full-page audit clean) · ☀/☾ edge chips CLICKABLE → bring body into view (FPV: camera.skyLook eased glide FPV.skyLookEaseTauMs, cancelled by direct look; orbit: heading/tilt glides, tilt only raised, cap 88). Tails: sub-M3 playback tick · MyPins wrapper live check (member cookie) · native time-picker during FF. `mem:project/wip-2026-07-14-uiux-qol-batch`; DECISIONS "2026-07-14 (LATEST — UI/UX QoL BATCH)". NEXT = owner A/B verdict (BLD chip) + the unchanged tails below. PRIOR: LOCAL-DEV TILES SHIPPED 2026-07-14 — `wix dev` streams tiles from THIS machine, NEVER R2 (0 workers.dev requests, browser-verified): bakes moved public/enriched/* → `bakes/enriched/*` (git-ignored) + `serve`-only Vite middleware (astro.config.mjs ftwLocalTiles serves /enriched/*) + NEW `.env.development.local` dev override (mode precedence beats .env.local); build/release keep the R2 URL (build-verified: 24 MB client bundle, no tiles, R2 URL baked — 372 MB under public/ would have shipped in every build). Both tilesets re-confirmed FULLY on R2 (dnipro-real-3 386 cells · dnipro-o2w-2 389 cells · manifests + glb magic). cities/*.json output → bakes/…; bake.mjs 1 cosmetic hint line; TRAP: @ts-check astro.config needs /** @returns {import(\"vite\").Plugin} */ on inline plugins. Gates astro 0/0 · vitest 530. NEXT = owner A/B verdict (BLD chip). PRIOR: 10 km EXTENT + BLD TOGGLE SHIPPED 2026-07-14 — both bakes cover GREATER Dnipro ~20×20 km (~10 km radius, bbox [34.915,48.37,35.185,48.55], grid 20 ≈1 km cells, versions dnipro-real-3/dnipro-o2w-2, both LIVE on R2) + a CameraTiltPanel `BLD` chip toggles CLASSIC↔OSM2WORLD in one click (reload-based by design, `#p=` pose hash keeps the view; helpers isVariantActive/toggleVariantUrl in enrichedVariant.ts). Numbers: classic 127,890 bldgs·386 tiles·149.14 MB·161,823 trees (maxTrees→200k, hash-uniform thinning would have hit in-city parks) · o2w 127,097 bldgs+5,662 constructions·7.71M verts·223.76 MB after DROPPING FENCES (PoleFence+ChainLinkFence measured 62% of ALL vertices — every post rendered, sub-pixel >1 km; dropClassesRegex knob, converts mtime-cached so the fix was adapter-only). Sync contract: dnipro.json bbox == tuning.ts ENRICHED.bbox — regen BOTH on change. Gates astro 0/0 · vitest 530 (+5) · browser-VERIFIED (verify-shots/o2w-10km-01..02: periphery pose ~8.9 km out + chip round-trip). NEXT = the owner's visual A/B verdict; carried: HLOD coarse tier (matters MORE — 20×20 km honest hole above ~20 km) · o2w untagged heights · draco 23× · grid-10 orphan R2 keys (harmless). `mem:project/wip-2026-07-14-osm2world-adapter` §10km-extent. PRIOR: OSM2WORLD PRODUCTION ADAPTER SHIPPED 2026-07-14 — a PARALLEL VARIANT live on R2 + a `?enriched=` same-pose A/B seam; NEXT = the OWNER'S visual verdict (o2w vs extruder), then draco/LOD size levers if o2w wins (`/frame`+investigate-design-v3, implement/Deep; browser-VERIFIED via Playwright MCP; astro check 0/0 · vitest 525 (+12); shots `verify-shots/o2w-adapter-01..05`). The CURRENT pipeline is UNTOUCHED (bake.mjs/dnipro.json/enriched-dnipro/default runtime byte-identical). NEW: `scripts/bake/bake-osm2world.mjs` + `lib/{osmXml,readGlb}.mjs` + `cities/dnipro-o2w.json` (extends dnipro → same bbox/grid/trees/mask; upload-r2 unchanged) + `src/lib/globe/enrichedVariant.ts` (resolveEnrichedUrl: no param→env byte-identical · off→Cesium OSM · name→segment swap · "/"→verbatim; pose in #hash → same-pose A/B on reload). LOAD-BEARING FIX: the spike's constant-offset re-base drifted ~2 km full-city (`>;` relation recursion → far origin) → adapter inverts OSM2World's MetricMapProjection EXACTLY per vertex (javap-disassembled: S=40075016.686·cos(lat0); x=((lon+180)/360)·S−x0; z=(ln((1+sinφ)/(1−sinφ))/4π+0.5)·S−y0; glTF=(x,ele,−z)) → 26,104/27,920 buildings in-bbox (extruder 26,569), 91 cells, densest = centre. Bake: 95 tiles · 3.89M verts · 110.22 MB (≈3.4× extruder — levers: weld+draco 23× needs DRACOLoader wiring, or --lod 1) · +3,112 constructions (fences/walls/lamps/pylons/cliffs) · SAME 24,714 trees (A/B differs only in structures) · R2 LIVE 97 files/105 MB `enriched/dnipro-o2w/` (Worker unchanged). Honest trade: untagged buildings take OSM2World's own (sometimes lower) heights vs our classDefaultsM. Reference-safe C6 in pure Node (ways/relations removed whole; tagged nodes keep geometry, lose tags). ALSO closed: the "globe paints from R2" tail (default view streams `dnipro-real-2` from the Worker). Traps: safe.osm rewrite kills the convert mtime cache · "Rail" regex ate "Railing" · wix dev idle SSR-error → restart. `mem:project/wip-2026-07-14-osm2world-adapter`; `dnipro-enrichment/OSM2WORLD_EXPERIMENT_PREP.md §10`. PRIOR: OSM2WORLD "SLICE 1.5" 1-CELL SPIKE DONE 2026-07-14 — verdict VIABLE, GREEN-LIGHT THE FULL ADAPTER (`/frame`+investigate-design-v3, implement/Deep, browser-VERIFIED in `wix dev` via Playwright MCP; NO tracked source touched → gates unchanged astro check 0/0 · vitest 513; shots `verify-shots/o2w-slice15-01..03`). All 4 DoD answered: axis `(east,up,−north)`==our `gv(e,u,−n)` CONFIRMED to certainty (adapter re-bin north=−Z lands 1362/1362=100% buildings inside bbox, +Z lands 0; +Y up, POSITION Y∈[−1,70] m) · ground Y≈0 seats on CWT (browser @646 m & @276 m building bases flush on the street grid) · per-building identity FREE via `keepOsmElements=true`+`extras.osmId`→contiguous `_feature_id_0` per building · buildings-only re-bin 2.91 MB/21 cells, densest cell (4,5) 1.219 MB raw (vs current baker 0.855 MB, ~1.4×) → weld+draco 52 KB (23×); the raw 25.66 MB glb was 89% ROADS (RoadModule + Overpass `>;` extent-recursion → filter `Building*` nodes). CORRECTIONS: distro is `0.5.0-SNAPSHOT` (modern `convert -i -o --config --lod` CLI + mandatory `--add-exports` on Java 21), origin=data-bounds centre (re-base by `projectEN(osmOrigin,ourBasis)` offset), no `_texture0_dir`⇒0 textures (COLOR_0 dropped), `excludeWorldModule` needs Road/Railway/SurfaceArea too. Owner's "other constructions" present (Wall/PoleFence/RetainingWall/ChainLinkFence/MobilePhoneMast/StreetLamp/BollardRow/AreaFountain). Enriched tileset = visual DROP-IN (buildingMaterial.ts swap works, 9 cells streamed, no shader/tile errors). NEXT = build `scripts/bake/bake-osm2world.mjs` (mirror bake.mjs; REUSE encodeGlb/buildTileset/geo.mjs/vegetation.mjs verbatim; promote spike `readGlb.mjs`→`lib/readGlb.mjs`; prod extract = Geofabrik PBF+`osmium extract`+`tags-filter --omit-referenced` C6; decide draco-decode-wiring vs ship-uncompressed + which construction classes). Spike scaffolding git-ignored under `scripts/bake/spike-osm2world/` (`readGlb`+`03-adapter` = the seed). `mem:project/wip-2026-07-14-osm2world-slice1.5-spike`; `.claude/claude-docs/dnipro-enrichment/OSM2WORLD_EXPERIMENT_PREP.md` §9. PRIOR: OWNER R2 HOSTING LIVE + OSM2WORLD PREP 2026-07-14 (ops/docs → gates unchanged astro check 0/0 · vitest 513): the Dnipro enriched tileset now streams from **Cloudflare R2 via the `frame-the-world` Worker** (CORS/Range, PRIVATE bucket, free-tier-safe, multi-city-extensible; `scripts/bake/{upload-r2,deploy-worker,r2-worker}.mjs`; `PUBLIC_ENRICHED_TILES_URL` points at it) + a deep **OSM2World experiment prep** doc (`.claude/claude-docs/dnipro-enrichment/OSM2WORLD_EXPERIMENT_PREP.md`; verdict viable, axis `(east,up,−north)` aligns with our `(e,u,−n)` for free) → NEXT = the OSM2World "Slice 1.5" 1-cell spike + confirm the globe paints from R2 in `wix dev` (`mem:project/wip-2026-07-14-r2-hosting-osm2world-prep`). PRIOR: OWNER BATCH SHIPPED 2026-07-14 later (browser-VERIFIED; astro check 0/0 · vitest 513 (+7) · wix build): PER-BUILDING/PER-TREE terrain re-seat (the sunk/levitating fix — `_feature_id_0` vertex runs + tree instanceMatrix[13] lifted on the CPU by terrain@footprint − cell seat; edges co-mutated via exact-position CSR; nearest-cell-first budgeted sampling; TWO garbage-sample guards incl. an apply-time poisoned-pair collapse — browser-caught −134 m transient; planFeed auto-invalidate once per settled seat epoch; live deltas settle −17.0…+18.7 m over 12k features + 7.4k trees) + always-on copyable viewer coords (`camera.camGeo` mirror → FpvHud POSITION row, 6 dp Google-Earth paste shape, `formatLatLonPaste`) + ALL floating panels draggable (`ui/DragGrip.tsx`, `--drag-x/--drag-y` CSS-var transform compose, session-remembered, dbl-click reset) + FPV ALTITUDE inverted (+rate = ascend) + FPV mini-map (`MINIMAP.patchM` 200 tunable; vectorTiles now parses the `building` layer; `scene/minimapFeed.ts` step 40 reuses the shared MVT cache → `store/minimap.ts` → `panels/MiniMap.tsx` north-up canvas w/ heading wedge). NEXT: owner R2 hosting (credentials ready per owner) · dayArcs skyline fold · WS4-D subject shadow timeline · optional HLOD · Phase 6 marketplace (`mem:project/wip-2026-07-14-owner-batch-seating-ui`). PRIOR: PASS 3 MOAT + SLICE 5 SHIPPED 2026-07-14 (browser-VERIFIED; astro check 0/0 · vitest 506 (+33) · wix build): the LIGHT PLANNER — pure `lib/ephemeris/planner.ts` (rise/set w/ dip · golden windows DERIVED from tuning.GOLDEN via asin of smoothstep-band mid-sines · culminations · full/new moon · `skylineState` scan+bisect over alt(t)−profile(az(t))) + `lib/geo/horizonProfile.ts` (az-bin max-elevation, terrain march d²(1−k)/2R, per-bin honesty/coverage) + `lib/geo/occlusion.ts` (**NO-Raycaster silhouette sweeps** — building triangle edges subdivided by segment CLOSEST APPROACH; slice-3 trees as canopy spheres from the EXT_mesh_gpu_instancing TRS, sidestepping their raycast noop; OSM verts rejected in the bboxClipPrismEcef mask prism since fragment clipping doesn't stop CPU rays) + `scene/planFeed.ts` step 40 (time-sliced 3 bins+2 meshes/frame; anchor photo-apex>FPV-eye>focus; 3 km trust; store/plan mirrors) + PlanPanel (PLAN pill → BEHIND SKYLINE/CLEAR + CLEARS/HIDES + jump-to-time chips → timeStore.setTime). LIVE: sun clears the real Dnipro skyline +37 min after astronomical sunrise (02:29Z vs 01:52Z); crossing precision 0.42° @+90 s; park skyline canopy-driven (trees ARE occluders — Slice 5 DoD). BUG fixed live: zenith overflow (grazing canopy painted ALL bins >90° via the 1/cos az spread → clamp altTop≤89.9/azHalf≤180 + test). TRAPS: `cmd | head -N` SIGPIPE-kills background wix dev · `#p=` low-alt restore glide exits temp-FPV (grab first, then setTempPin+setTempFpv) · sampleProfile lerps bin CENTRES (march at (i+0.5)·binWidth). UNVERIFIED tails: photo-apex anchor in-browser · landmark-height validation · dayArcs skyline fold (seam: planFeed.profileSample()). NEXT: owner R2 hosting · dayArcs fold · WS4-D subject shadow timeline · optional HLOD · Phase 6 marketplace (`mem:project/wip-2026-07-14-pass3-obstruction-moat`). PRIOR: SLICE 3 TREES SHIPPED 2026-07-13 (browser-VERIFIED; astro check 0/0 · vitest 473 · wix build): ~24.7k deterministic trees (418 OSM points + tree_row sampling + seeded wood/park scatter, building/C6/bbox rejection, hash-seeded reproducible) baked as `EXT_mesh_gpu_instancing` nodes INSIDE the enriched cell glbs (~50 B/tree, 94 tiles · 34.05 MB total) — three 0.185 loads ONE InstancedMesh/cell (default extension, source-verified), inheriting streaming/LRU/per-cell re-seat; shared vecGreen flat-shaded night-dimmed shadow-casting material in enrichedBuildings.ts + TREES tuning group. TRAPS (source-verified): tile disposal never calls mesh.dispose() → instanceMatrix GL buffer leaks per LRU eviction (dispose-model now calls c.dispose(); LRU cycle verified clean) · InstancedMesh.raycast iterates every instance → raycast noop on tree meshes. Terrain decision RECORDED: keep CWT. Canopy rasters = README upgrade tier. NEXT: Slice 5 obstruction moat · owner R2 hosting · optional HLOD (`mem:project/wip-2026-07-13-dnipro-slice3-trees`). PRIOR: SLICE 0 BROWSER-VERIFIED + SLICE 1 REAL BAKE SHIPPED (2026-07-13; gates astro check 0 · vitest 442 · wix build; browser-VERIFIED in wix dev via Playwright): a reproducible Node DATA-DRIVEN baker `scripts/bake` (`npm run bake -- --city dnipro`) reconstructs real Dnipro buildings from OSM (footprints + inferred heights `levels×3m` + roof shapes flat/gable/pyramid + C6 military/critical-infra exclusion) → spatial-grid 3D-Tiles (per-cell glb, shared ENU frame, `_feature_id_0` per building) → masks & streams over Dnipro (3233 buildings/16 tiles/172k verts/4.83 MB, seated `group lift 101.3≈terrain 101.3` + masked + streaming, native-edge = seamless OSM extension). Owner picked the Node path over OSM2World/heavy-3dfier (Docker daemon down + public npm blocked here; both = documented upgrade tiers in `scripts/bake/README.md`). OWNER FEEDBACK 2026-07-13 (all browser-VERIFIED): bake EXPANDED to ~5.9×6.3 km / 46 tiles (both Dnipro banks) + FPV 500 mm max focal + arrow-key WALK + a BUILDINGS wireframe↔solid slider. OWNER #4 TERRAIN ELEVATION / PER-CELL RE-SEAT SHIPPED 2026-07-13 (browser-VERIFIED; astro check 0 · vitest 448 · wix build): each grid cell now offsets along its own geodetic up by (terrain@cell-centre − centre seat) — measured spread −20.6 m riverbank → +82.0 m hills over 102 m of real CWT relief, offsets match terrain within ~0.2 m; TRAP: TilesGroup.updateMatrixWorld skips children unless the GROUP matrix changed → per-cell writes call scene.updateMatrixWorld(true); baker pads region heights ±80 m (`mem:project/wip-2026-07-13-terrain-reseat`). SLICE 2 COMPLETE 2026-07-13 (browser-VERIFIED; astro check 0 · vitest 459 · wix build): (b) clip-hole — `bboxClipPrismEcef` 4-plane ECEF prism + clipIntersection/clipShadows on the shared OSM materials fixes the straddle-leak pixel-exactly (box core clean at 28 km; enriched instance carries no planes); (c) stylization reconciled — NEW `scene/buildingMaterial.ts` factory (R2 tone via `_feature_id_0`, F1 reveal, ghost, dormant R3) consumed by BOTH tilesets as SEPARATE instances (a shared one would clip the enriched set inside its own prism), enriched gains setNight, debugDistinctEdges=false; (e) FULL-CITY bake [35.00,48.42,35.10,48.50] grid 10 → 26,569 bldgs · 90 tiles · 1.17M verts · 32.8 MB, streams + LRU re-streams; (d) R2 tooling — pure-Node SigV4 `scripts/bake/upload-r2.mjs` + `lib/s3sign.mjs`, dry-run-verified; ACTUAL bucket/custom-domain/CORS = OWNER Cloudflare action (r2.dev has no CORS). TRAP: extracting module-level consts to a new module leaves the running dev server serving a stale HMR transform → runtime ReferenceError with a clean on-disk file — restart wix dev. KNOWN LIMIT: no HLOD → above ~20 km the bbox renders honestly EMPTY of buildings. NEXT — Slice 3 trees → owner R2 hosting → optional HLOD tier → Slice 5 feed the Pass-3 obstruction moat (`mem:project/wip-2026-07-13-dnipro-slice2`). PRIOR Slice-0 integration was CODE-COMPLETE (DEFAULT-OFF behind `PUBLIC_ENRICHED_TILES_URL`): masking OSM in-bbox (stop-traversal `calculateTileViewError` plugin) + a 3rd `TilesRenderer` (plain-URL + `GLTFExtensionsPlugin`) + R1 runtime clamp-to-CWT seating (CWT ellipsoidal vs GLO-30/EGM2008 orthometric, Dnipro undulation +20.42 m) — locally gated (astro check 0 · vitest 422 · wix build); NEXT = browser-verify the hand-baked sample (`node scripts/build-sample-dnipro-tiles.mjs` → `PUBLIC_ENRICHED_TILES_URL=/enriched-sample/dnipro/tileset.json` → `wix dev`) then the real OSM2World bake + R2 (`mem:project/wip-2026-07-13-dnipro-slice0-spike` + `dnipro-enrichment/DNIPRO_SLICE0_SPIKE.md`). ALSO 2026-07-13 — ILLUMINATION PASS code-complete + locally gated (astro check 0 · vitest 422 · wix build), BROWSER-UNVERIFIED: crisper shadows (DRAPE.shadowOpacity 0.62→0.80 is the key — the default dark-drape city view blends toward it) + richer sun/moon golden-hour GI (widened GOLDEN band + strengths + NEW keyBrighten/moonKeyStrength) + brighter dark-of-moon night. The cyberpunk WINDOW GRID I briefly added was REMOVED at owner request ("looks junky") — buildings carry NO night emissive; their identity = dark mass + lit edges + cast/received SHADOWS + golden/moon key. Owner's live ask = buildings casting NICE shadows on ground + each other (SHADOWS.normalBias 1.0→0.75 anchors them, radius→2). Illumination + shadows are now owner-VERIFIED ("working"): the "no shadows" bug was the frame GOVERNOR throttling the M3 Pro to tier `low` (which disables the shadow pass) — fixed by decoupling shadows from the governor (follow DEVICE tier) + flooring a strong device at `mid`; plus crisp shadows + sun/moon golden-hour GI (`mem:project/wip-2026-07-13-illumination-pass`). **NEXT INITIATIVE: the FULL Dnipro 3D enrichment implementation (Slices 1→5 of `dnipro-enrichment/DNIPRO_3D_ENRICHMENT_PLAN.md`) — the real bake (OSM2World / Overture+height-inference) → glb→3D-Tiles → self-host on R2 → mask & stream over Dnipro → trees → feed the Pass-3 obstruction moat. Slice-0 integration is code-complete + DEFAULT-OFF; handover rewritten in `NEXT_SESSION_PROMPT.md`.** Prior context: an offline BAKE of roof-shaped LOD2 buildings self-hosted on Cloudflare R2, masking Cesium OSM Buildings inside the Dnipro bbox (external research 2026-07-13 INCORPORATED → `.claude/claude-docs/dnipro-enrichment/DNIPRO_3D_ENRICHMENT_PLAN.md` + `mem:project/wip-2026-07-13-dnipro-enrichment-research`; reframes/retires R4 — bake-and-mask replaces buildings in-bbox, sidestepping the z-fight/cull blocker; extends D1/D13/C6/C1, all data $0). Recommended DEFAULTS (owner dismissed the fork Q): SLICE 0 de-risk spike FIRST (prove masking + R1 vertical-datum seating vs rendered Cesium World Terrain + R2/CORS + stylization reuse), then the full bake; hero splats deferred; the Pass 1/2 browser-verify + GTAO fold into the next browser session. FOLDED-IN prior headline: RENDERING PASS 2 (Dnipro identity) R2+R3 CODE-COMPLETE (2026-07-13) — per-building tonal
...
See `archive/ARCHITECTURE_REVIEW.md` + `NEXT_SESSION_PROMPT.md` + `mem:project/wip-2026-07-11-pre-s7-refactor-s2`.
